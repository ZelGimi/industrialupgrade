...
public abstract class TileEntityAirPanel extends TileEntityInventory implements IEnergySource, IHasGui, IGuiValueProvider {
  ...
  public TileEntityAirPanel(SolarConfig config) {
    this(config.dayPower, config.nightPower, config.storage, config.tier);
  }
  
  public TileEntityAirPanel(int dayPower, int nightPower, int storage, int tier) {
    this.dayPower = dayPower;
    this.nightPower = nightPower;
    ...
    this.dayPower1=(int) (dayPower*1.5);
  ...
  public ContainerBase<? extends TileEntityAirPanel> getGuiContainer(EntityPlayer player) {
    return (ContainerBase<? extends TileEntityAirPanel>)DynamicContainer.create(this, player, GuiParser.parse(this.teBlock));
  }
