...
public abstract class TileEntityEnderPanel extends TileEntityInventory implements IEnergySource, IHasGui, IGuiValueProvider {
  ...
  public TileEntityEnderPanel(SolarConfig config) {
    this(config.dayPower, config.nightPower, config.storage, config.tier);
  }
  
  public TileEntityEnderPanel(int dayPower, int nightPower, int storage, int tier) {
    this.dayPower = dayPower;
    this.nightPower = nightPower;
  ...
  public ContainerBase<? extends TileEntityEnderPanel> getGuiContainer(EntityPlayer player) {
    return (ContainerBase<? extends TileEntityEnderPanel>)DynamicContainer.create(this, player, GuiParser.parse(this.teBlock));
  }
