...
public class ItemBeeAnalyzer extends Item implements IItemStackInventory, IModelRegister {
    ...
    public boolean onDroppedByPlayer(@Nonnull ItemStack stack, EntityPlayer player) {
        if (!player.getEntityWorld().isRemote && !ModUtils.isEmpty(stack) && player.openContainer instanceof ContainerBeeAnalyzer) {
            ItemStackBeeAnalyzer toolbox = ((ContainerBeeAnalyzer) player.openContainer).base;
    ...
    public void onUpdate(
        ...
        if (nbt.getBoolean("open")) {
            ...
            if (slot_id != itemSlot && !player.getEntityWorld().isRemote && !ModUtils.isEmpty(stack) && player.openContainer instanceof ContainerBeeAnalyzer) {
                ItemStackBeeAnalyzer toolbox = ((ContainerBeeAnalyzer) player.openContainer).base;
                if (toolbox.isThisContainer(stack)) {
