...
public class ScreenBook<T extends ContainerMenuBook> extends ScreenMain<ContainerMenuBook> {
    ...
    public ScreenBook(Player player, final ItemStack itemStack1, final ContainerMenuBook containerBook) {
        ...
        if (container.base.itemStack1.has(DataComponentsInit.BOOKMARK)) {
            listBookMark = container.base.itemStack1.get(DataComponentsInit.BOOKMARK);
        }
    ...
    public void addBookMark(int tab, int id) {
        ...
        container.base.itemStack1.set(DataComponentsInit.BOOKMARK, listBookMark);
        bookMarksSize = calculateGrid(listBookMark.size());
    ...
    public void drawForegroundLayer(GuiGraphics poseStack, int par1, int par2) {
        ...
        for (int index = 0; index < guideTabs.size(); index++) {
            ...
            if (isSelectedTab) {
                new TooltipWidget(this, -33, y, 33, 27).withTooltip(Localization.translate(guideTab.name))
                        .drawForeground(poseStack, par1, par2);
            } else {
                new TooltipWidget(this, dx, y, w, 27).withTooltip(Localization.translate(guideTab.name))
                        .drawForeground(poseStack, par1, par2);
            }
        ...
        if (!bookMark) {
            ...
            if (map != null) {
                ...
                if (par1 >= 8 && par2 >= 9 && par1 <= 238 + 9 && par2 <= 176 + 9) {
                    for (Quest quest : GuideBookCore.instance.getQuests(tab)) {
                        ...
                        new TooltipWidget(this, centerX + quest.x, centerY + quest.y, texW, texW).withTooltip(quest.getLocalizedName())
                                .drawForeground(poseStack, par1, par2);
                    }
                }
            }
            ...
            for (int i = 0; i < listBookMark.size(); i++) {
                ...
                Quest quest = GuideBookCore.instance.getQuests(tuple.getA()).get(tuple.getB());
                int texW = (quest.shape == Shape.EPIC) ? 26 : 24;
                new TooltipWidget(this, x1, y1, texW, texW).withTooltip(quest.getLocalizedName())
                        .drawForeground(poseStack, par1, par2);
            }
        }
    }
    ...
    public void drawGuiContainerBackgroundLayer(GuiGraphics poseStack, float partialTicks, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < guideTabs.size(); i++) {
            ...
            if (isSelectedTab) {
                ...
                new ItemWidget(this, -33 + 9, y + 6, () -> guideTab.icon)
                ...
                if (!bookMark) {
                    for (Quest quest : GuideBookCore.instance.getQuests(i)) {
                        ...
                        new ItemWidget(this, centerX + quest.x + 7, centerY + quest.y + 7, () -> quest.icon)
                                .drawBackground(poseStack, guiLeft, guiTop);
                    }
                }
                disableScissor();
                ...
                new ItemWidget(this, dx + 8, y + 6, () -> guideTab.icon)
                        .drawBackground(poseStack, guiLeft, guiTop);
            }
        ...
        if (bookMark) {
            ...
            for (int i = 0; i < listBookMark.size(); i++) {
                ...
                new ItemWidget(this, x1 - guiLeft + 4, y1 - guiTop + 4, () -> quest.icon)
                        .drawBackground(poseStack, guiLeft, guiTop);
            }
            disableScissor();
        }
