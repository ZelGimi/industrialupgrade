...
public class ItemAmpereCable extends Item implements ISubItem<AmpereType>, IModelRegister {

    public static final List<ItemStack> variants = new ArrayList<>();
    protected static final String NAME = "ae_iu_item";
    String[] name = {"acable"};


    public ItemAmpereCable() {
        super();
        this.setHasSubtypes(true);
        AmpereType[] var1 = AmpereType.values;


        for (AmpereType type : var1) {
    ...
    public static ModelResourceLocation getModelLocation(ItemStack stack) {
        final String loc = Constants.MOD_ID +
                ':' +
                "acable" + "/" + getName(stack);

        return new ModelResourceLocation(loc, null);
    }

    private static AmpereType getCableType(ItemStack stack) {
        int type = stack.getItemDamage();
        return type < AmpereType.values.length ? AmpereType.values[type] : AmpereType.acable;
    }

    private static int getInsulation(ItemStack stack) {
        AmpereType type = getCableType(stack);
    ...
    protected void registerModel(final int meta, final String extraName) {
        ModelLoader.setCustomModelResourceLocation(
                ...
                new ModelResourceLocation(
                        Constants.MOD_ID + ":" + NAME + "/" + AmpereType.values[meta].getName(),
                        null
                )
        );
    }

    public ItemStack getItemStack(AmpereType type) {
        return getCable(type);
    }

    public ItemStack getItemStack(String variant) {
        int pos = 0;
        AmpereType type = null;
    ...
    public ItemStack getCable(AmpereType type) {
    ...
    public EnumActionResult onItemUse(
        ...
        if (!ModUtils.isEmpty(stack) && player.canPlayerEdit(pos, side, stack) && world.mayPlace(
            ...
            TileEntityAmpereCable te;
            te = TileEntityAmpereCable.delegate(type);
    ...
    public void getSubItems(@Nonnull CreativeTabs tab, @Nonnull NonNullList<ItemStack> itemList) {
        if (this.isInCreativeTab(tab)) {
            List<ItemStack> variants = new ArrayList<>(ItemAmpereCable.variants);
    ...
    public Set<AmpereType> getAllTypes() {
        return EnumSet.allOf(AmpereType.class);
    }
