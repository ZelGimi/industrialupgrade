...
public class ItemSteamPipe extends Item implements ISubItem<SteamType>, IModelRegister {

    public static final List<ItemStack> variants = new ArrayList<>();
    protected static final String NAME = "spipe_item";
    String[] name = {"spipe"};
    ...
    public ItemSteamPipe() {
        ...
        SteamType[] var1 = SteamType.values;


        for (SteamType type : var1) {
    ...
    private static SteamType getCableType(ItemStack stack) {
        int type = stack.getItemDamage();
        return type < SteamType.values.length ? SteamType.values[type] : SteamType.spipe;
    }

    private static int getInsulation(ItemStack stack) {
        SteamType type = getCableType(stack);
    ...
    protected void registerModel(final int meta, final String extraName) {
        ModelLoader.setCustomModelResourceLocation(
                ...
                new ModelResourceLocation(
                        Constants.MOD_ID + ":" + NAME + "/" + SteamType.values[meta].getName(),
                        null
                )
        );
    }

    public ItemStack getItemStack(SteamType type) {
    ...
    public ItemStack getCable(SteamType type) {
    ...
    public EnumActionResult onItemUse(
        ...
        if (!ModUtils.isEmpty(stack) && player.canPlayerEdit(pos, side, stack) && world.mayPlace(
            ...
            TileEntitySteamPipe te;
            te = TileEntitySteamPipe.delegate(type);
    ...
    public void getSubItems(@Nonnull CreativeTabs tab, @Nonnull NonNullList<ItemStack> itemList) {
        if (this.isInCreativeTab(tab)) {
            List<ItemStack> variants = new ArrayList<>(ItemSteamPipe.variants);
    ...
    public Set<SteamType> getAllTypes() {
        return EnumSet.allOf(SteamType.class);
    }
