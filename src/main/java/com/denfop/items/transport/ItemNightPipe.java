...
import com.denfop.tiles.transport.tiles.TileEntityNightPipe;
...
public class ItemNightPipe extends Item implements ISubItem<NightType>, IModelRegister {

    public static final List<ItemStack> variants = new ArrayList<>();
    protected static final String NAME = "npipe_item";
    String[] name = {"npipe"};


    public ItemNightPipe() {
        super();
        this.setHasSubtypes(true);
        NightType[] var1 = NightType.values;


        for (NightType type : var1) {
    ...
    private static NightType getCableType(ItemStack stack) {
        int type = stack.getItemDamage();
        return type < NightType.values.length ? NightType.values[type] : NightType.npipe;
    }

    private static int getInsulation(ItemStack stack) {
        NightType type = getCableType(stack);
    ...
    protected void registerModel(final int meta, final String extraName) {
        ModelLoader.setCustomModelResourceLocation(
                ...
                new ModelResourceLocation(
                        Constants.MOD_ID + ":" + NAME + "/" + NightType.values[meta].getName(),
                        null
                )
        );
    }

    public ItemStack getItemStack(NightType type) {
    ...
    public ItemStack getCable(NightType type) {
    ...
    public EnumActionResult onItemUse(
        ...
        if (!ModUtils.isEmpty(stack) && player.canPlayerEdit(pos, side, stack) && world.mayPlace(
            ...
            TileEntityNightPipe te;
            te = TileEntityNightPipe.delegate(type);
    ...
    public void getSubItems(@Nonnull CreativeTabs tab, @Nonnull NonNullList<ItemStack> itemList) {
        if (this.isInCreativeTab(tab)) {
            List<ItemStack> variants = new ArrayList<>(ItemNightPipe.variants);
    ...
    public Set<NightType> getAllTypes() {
        return EnumSet.allOf(NightType.class);
    }
