...
public class ItemBioPipe extends Item implements ISubItem<BioType>, IModelRegister {
    ...
    public ItemBioPipe() {
        ...
        BioType[] var1 = BioType.values;


        for (BioType type : var1) {
    ...
    private static BioType getCableType(ItemStack stack) {
        int type = stack.getItemDamage();
        return type < BioType.values.length ? BioType.values[type] : BioType.bpipe;
    }

    private static int getInsulation(ItemStack stack) {
        BioType type = getCableType(stack);
    ...
    protected void registerModel(final int meta, final String extraName) {
        ModelLoader.setCustomModelResourceLocation(
                ...
                new ModelResourceLocation(
                        Constants.MOD_ID + ":" + NAME + "/" + BioType.values[meta].getName(),
                        null
                )
        );
    }

    public ItemStack getItemStack(BioType type) {
    ...
    public ItemStack getCable(BioType type) {
    ...
    public EnumActionResult onItemUse(
        ...
        if (!ModUtils.isEmpty(stack) && player.canPlayerEdit(pos, side, stack) && world.mayPlace(
            ...
            TileEntityBioPipe te;
            te = TileEntityBioPipe.delegate(type);
    ...
    public void getSubItems(@Nonnull CreativeTabs tab, @Nonnull NonNullList<ItemStack> itemList) {
        if (this.isInCreativeTab(tab)) {
            List<ItemStack> variants = new ArrayList<>(ItemBioPipe.variants);
