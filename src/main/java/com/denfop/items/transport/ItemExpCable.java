...
public class ItemExpCable extends ItemIC2 implements IMultiItem<ExpType>, IBoxable, IModelRegister {
    ...
    public static ModelResourceLocation getModelLocation(ItemStack stack) {
        final String loc = Constants.MOD_ID +
                ':' +
                "expcable" + "/" + getName(stack);

        return new ModelResourceLocation(loc, null);
    }

    private static ExpType getCableType(ItemStack stack) {
        int type = stack.getItemDamage();
        return type < ExpType.values.length ? ExpType.values[type] : ExpType.expcable;
    }
    ...
    public void registerModels() {
        ...
        ModelLoader.setCustomMeshDefinition(this, ItemExpCable::getModelLocation);
    ...
    protected void registerModel(final int meta, final ItemName name, final String extraName) {
        ModelLoader.setCustomModelResourceLocation(
                ...
                new ModelResourceLocation(
                        Constants.MOD_ID + ":" + NAME + "/" + ExpType.values[meta].getName(),
                        null
                )
        );
    }

    public ItemStack getItemStack(ExpType type) {
    ...
    public ItemStack getCable(ExpType type) {
    ...
    public void getSubItems(@Nonnull CreativeTabs tab, @Nonnull NonNullList<ItemStack> itemList) {
        if (this.isInCreativeTab(tab)) {
            List<ItemStack> variants = new ArrayList<>(ItemExpCable.variants);
    ...
    public Set<ExpType> getAllTypes() {
        return EnumSet.allOf(ExpType.class);
    }
