...
public final class JetpackVisualClient {
    ...
    public static void tick(
        ...
        if (!(player.getLevel() instanceof ClientLevel level)) {
            return;
        }
        ...
        final boolean activeNow = thrustActive || (jetpackEnabled && !player.isOnGround());
    ...
    private static State detectState(
        ...
        if (thrustActive) {
            if (verticalSpeed > 0.10D || (jumpPressed && !player.isOnGround())) {
                return State.ASCENT;
            }
    ...
    private static Vec3 resolveExhaustDirection(
        ...
        Vec3 direction = new Vec3(
                -horizontalMotion.x * (0.30D + Math.min(horizontalMotion.length(), 0.60D) * 0.50D),
                -1.0D,
                -horizontalMotion.z * (0.30D + Math.min(horizontalMotion.length(), 0.60D) * 0.50D)
        );
