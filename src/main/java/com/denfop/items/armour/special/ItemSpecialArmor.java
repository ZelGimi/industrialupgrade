...
public class ItemSpecialArmor extends ArmorItem implements ISpecialArmor, IItemStackInventory, IEnergyItem,
    ...
    public ItemSpecialArmor(EnumSubTypeArmor subTypeArmor, EnumTypeArmor typeArmor) {
        super(ArmorMaterials.ENERGY_ITEM, subTypeArmor.getEntityEquipmentSlot(), new Properties().tab(IUCore.EnergyTab).stacksTo(1).setNoRepair());
        ...
        switch (this.slot) {
            case HEAD:
                ...
                if (Nightvision && !world.isClientSide &&
                    ...
                    int skylight = player.getLevel().getMaxLocalRawBrightness(pos);
            ...
            case LEGS:
                ...
                if (this.listCapability.contains(EnumCapability.SPEED) && ElectricItem.manager.canUse(
                        itemStack,
                        1000.0D
                ) && (player.isOnGround() || player.isInWater()) && player.isSprinting()) {
            ...
            case FEET:
                if (!world.isClientSide()) {
                    ...
                    if (wasOnGround && !player.isOnGround() && IUCore.keyboard
                    ...
                    if (player.isOnGround() != wasOnGround) {
                        nbtData.putBoolean("wasOnGround", player.isOnGround());
                    }
                    if (ElectricItem.manager.canUse(itemStack, 4000.0D) && player.isOnGround()) {
...
*/
    ...
    public void onEntityLivingFallEvent(LivingFallEvent event) {
        if (!event.getEntity().getLevel().isClientSide() && event.getEntity() instanceof LivingEntity) {
    ...
    public ArmorProperties getProperties(
        ...
        if (source == DamageSource.FALL && this.slot.getIndex() == 0) {
