...
public class PresetColorScrollWidget extends AbstractWidget {
    ...
    public void renderButton(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        for (int index = startIndex; index < endIndex; index++) {
            ...
            int cellX = startX + col * (SWATCH + GAP);
            int cellY = startY + row * (SWATCH + GAP);
            int rgb = this.colors.get(index) & 0xFFFFFF;

            ColorPickerRenderUtil.drawInsetPanel(poseStack, cellX, cellY, SWATCH, SWATCH, 0xFF161B22, 0xFF2C3441);
            ColorPickerRenderUtil.drawCheckerboard(poseStack, cellX + 1, cellY + 1, SWATCH - 2, SWATCH - 2);
            fill(poseStack, cellX + 1, cellY + 1, cellX + SWATCH - 1, cellY + SWATCH - 1, 0xFF000000 | rgb);

            boolean hovered = mouseX >= cellX && mouseX < cellX + SWATCH && mouseY >= cellY && mouseY < cellY + SWATCH;
            if (hovered) {
                ColorPickerRenderUtil.drawBorder(poseStack, cellX, cellY, SWATCH, SWATCH, 0xFFFFFFFF);
            } else if (rgb == this.selectedColor) {
                ColorPickerRenderUtil.drawBorder(poseStack, cellX, cellY, SWATCH, SWATCH, 0xFF8DEBFF);
            }
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        int localX = (int) mouseX - (this.x + 4);
