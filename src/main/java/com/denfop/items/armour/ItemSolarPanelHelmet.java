...
public class ItemSolarPanelHelmet extends ItemArmorEnergy implements EnergyItem, ISpecialArmor,
    ...
    public void onArmorTick(@Nonnull ItemStack itemStack, Level worldObj, @Nonnull Player player) {
        ...
        if (Nightvision && !player.getLevel().isClientSide() &&
                ElectricItem.manager.use(itemStack, 1.0D, player)) {
            int x = Mth.floor(player.getX());
            int z = Mth.floor(player.getZ());
            int y = Mth.floor(player.getY());
            int skylight = player.getLevel().getMaxLocalRawBrightness(new BlockPos(x, y, z));
    ...
    public void gainFuel(Player player) {
        if (player.getLevel().getGameTime() % 128 == 0) {
    ...
    public void updateVisibility(Player player) {
        ...
        boolean noSunWorld = !(player.getLevel().dimension() == Level.OVERWORLD);
        boolean rainWeather = player.getLevel().isRaining() || player.getLevel().isThundering();
        this.sunIsUp = player.getLevel().isDay() && !rainWeather;

        this.skyIsVisible = player.getLevel().canSeeSky(pos.above()) &&
                (player.getLevel().getBlockState(pos.above()).getMaterial().getColor() ==
                        MaterialColor.NONE) && !noSunWorld;
    }
