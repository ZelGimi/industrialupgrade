...
public class ItemAdvJetpack extends ItemArmorEnergy implements EnergyItem, ISpecialArmor, IProperties, UpgradeItem {
    ...
    public void inventoryTick(ItemStack stack, Level worldIn, Entity entity, int slot, boolean selected) {
        super.inventoryTick(stack, worldIn, entity, slot, selected);
    ...
    private void dispatchClientJetpackVisuals(
        ...
        final boolean hoverThrust = hoverMode && (jumpPressed || descendPressed || !player.isOnGround());
    ...
    public void onArmorTick(@Nonnull ItemStack itemStack, @Nonnull Level world, Player player) {
        ...
        if (IUCore.keyboard.isJumpKeyDown(player) && IUCore.keyboard.isVerticalMode(player) && toggleTimer == 0) {
            ...
            if (!player.getLevel().isClientSide()) {
                ...
                if (hoverMode) {
                    IUCore.proxy.messagePlayer(player, Localization.translate("iu.message.hover_mode.enabled"));
                } else {
                    IUCore.proxy.messagePlayer(player, Localization.translate("iu.message.hover_mode.disabled"));
                }
            }
        }
            ...
            if (!world.isClientSide) {
                if (!ElectricItem.manager.canUse(itemStack, 1.0D)) {
                    ...
                    if (ElectricItem.manager.canUse(itemStack, 25.0D)) {
                        ElectricItem.manager.use(itemStack, 25.0D, player);
                        jetpackUsed = true;
                    } else {
                        nbtData.putBoolean("jetpack", false);
                    }
