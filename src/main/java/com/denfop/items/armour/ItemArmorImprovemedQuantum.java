...
public class ItemArmorImprovemedQuantum extends ItemArmorEnergy
        implements IModelRegister, IUpgradeItem, ISpecialArmor, IHazmatLike {
    ...
    public ItemArmorImprovemedQuantum(
        ...
        super("", armorType1, maxCharge1, transferLimit1 * 16, tier1);
    ...
    public String getUnlocalizedName() {
        return "supersolarpanels." + super.getUnlocalizedName().substring(3);
    }
    ...
    public void onArmorTick(@Nonnull World world, @Nonnull EntityPlayer player, @Nonnull ItemStack itemStack) {
        ...
        switch (this.armorType.getIndex()) {
            ...
            case 2:
                ...
                if (IUCore.keyboard.isChangeKeyDown(player) && IUCore.keyboard.isSneakKeyDown(player) && toggleTimer == 0) {
                    toggleTimer = 10;
                    if (IUCore.proxy.isSimulating()) {
                        if (magnet) {
                ...
                if (!player.getEntityWorld().isRemote && player.getEntityWorld().provider.getWorldTime() % 40 == 0) {
                    for (int i = 0; i < player.inventory.armorInventory.size(); i++) {
                        ...
                        if (!player.inventory.armorInventory.get(i).isEmpty() && player.inventory.armorInventory
                                .get(i)
                                .getItem() instanceof IEnergyItem && player.inventory.armorInventory
                                .get(i)
                                .getItem() != this) {
            ...
            case 1:
                ...
                if (ElectricItem.manager.canUse(
                        itemStack,
                        1000.0D
                ) && (player.onGround || player.isInWater()) && IUCore.keyboard.isForwardKeyDown(player) && (player.isSprinting() && IUCore.keyboard.isChangeKeyDown(
                        player))) {
