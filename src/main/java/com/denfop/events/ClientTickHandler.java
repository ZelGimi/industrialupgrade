...
public class ClientTickHandler {
    public static void onTickRender1(PoseStack guiGraphics) {
        ...
        if (shouldDisplayRadiationInfo(player)) {
            ...
            mc.font.draw(guiGraphics, radiationText, 40, 90, 0xFFFFFF);

            RenderSystem.setShaderTexture(0, new ResourceLocation(Constants.MOD_ID, "textures/gui/gui_dosimeter.png"));
            guiGraphics.scale(0.4f, 0.4f, 0.4f);
            mc.screen.blit(guiGraphics, 0, 10, 0, 0, 256, 256, 256, 256);


            ResourceLocation radiationTexture = getRadiationTexture(player);
            RenderSystem.setShaderTexture(0, radiationTexture);
            mc.screen.blit(guiGraphics, 0, 10, 0, 0, 256, 256, 256, 256);
            guiGraphics.popPose();
        }
    }
