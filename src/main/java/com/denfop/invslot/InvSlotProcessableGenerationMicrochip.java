...
public class InvSlotProcessableGenerationMicrochip extends InvSlotProcessable {
    ...
    public InvSlotProcessableGenerationMicrochip(TileEntityInventory base1, String name1, int count) {
    ...
    public boolean accepts(ItemStack itemStack) {
        for (Map.Entry<IMicrochipFarbricatorRecipeManager.Input, RecipeOutput> entry : getRecipeList().entrySet()) {
            if (entry.getKey().container.matches(itemStack)
                    || entry.getKey().fill.matches(itemStack) || entry.getKey().fill1.matches(itemStack) || entry.getKey().container1.matches(
                    itemStack) || entry.getKey().fill2.matches(itemStack)) {
