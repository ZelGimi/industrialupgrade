...
import com.denfop.blockentity.chemicalplant.BlockEntityChemicalPlantSeparate;
...
public class ScreenChemicalSeparate<T extends ContainerMenuDefaultMultiElement> extends ScreenMain<ContainerMenuDefaultMultiElement> {
    ...
    public ScreenChemicalSeparate(ContainerMenuDefaultMultiElement guiContainer) {
        ...
        this.addWidget((new FluidDefaultWidget(
                ...
                20,
                ((BlockEntityChemicalPlantSeparate) guiContainer.base).getFluidTank().getFluid()
        ) {
            @Override
            public void drawBackground(PoseStack poseStack, final int mouseX, final int mouseY) {
                bindCommonTexture();
                FluidStack fs = ((BlockEntityChemicalPlantSeparate) guiContainer.base).getFluidTank().getFluid();
