...
public class ScreenWirelessMineralQuarry<T extends ContainerMenuWirelessMineralQuarry> extends ScreenMain<ContainerMenuWirelessMineralQuarry> {


    public ScreenWirelessMineralQuarry(ContainerMenuWirelessMineralQuarry guiContainer) {
        super(guiContainer);
    ...
    protected void drawGuiContainerBackgroundLayer(PoseStack poseStack, final float partialTicks, final int mouseX, final int mouseY) {
        ...
        for (int i = 0, j = 0; i < 4; i++) {
            if (!this.container.base.invslot.get(i).isEmpty() && j < this.container.base.veinList.size()) {
                ...
                stack.setCount(1);
                new ItemStackWidget(this, 130, 8 + i * 18, () -> stack).drawBackground(poseStack,
                        guiLeft,
                        guiTop
                );
                j++;
