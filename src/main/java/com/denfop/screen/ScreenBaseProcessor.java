...
public class ScreenBaseProcessor<T extends ContainerBaseProcessor> extends ScreenMain<ContainerBaseProcessor> {
    ...
    public ScreenBaseProcessor(ContainerBaseProcessor guiContainer) {
        super(guiContainer);
        this.imageWidth = 255;
    ...
    public void updateTickInterface() {
        ...
        maxValue = container.base.getCraftOutputs().size() - 6;
        if (maxValue < 0) {
            maxValue = 0;
        }
        if (value > maxValue) {
    ...
    protected void drawForegroundLayer(PoseStack poseStack, final int par1, final int par2) {
        ...
        if (!this.container.base.hasSystem || this.container.base.getCraftOutputs().isEmpty()) {
            return;
        }

        List<AutoCraftOutput> outputs = new ArrayList<>(this.container.base.getCraftOutputs());
        int startIndex = value;
        int endIndex = Math.min(startIndex + 6, outputs.size());

        for (int i = value; i < endIndex; i++) {
            AutoCraftOutput step = outputs.get(i);
            if (step == null) {
                continue;
            }
    ...
    protected void mouseClicked(int i, int j, int k) {
        ...
        if (this.container.base.getCraftOutputs().isEmpty()) {
            return;
        }

        ArrayList<AutoCraftOutput> outputs = new ArrayList<>(this.container.base.getCraftOutputs());
        int startIndex = value;
        int endIndex = Math.min(startIndex + 6, outputs.size());
        ...
        for (int ii = startIndex; ii < endIndex; ii++) {
            ...
            if (inX && inY) {
                new PacketUpdateServerTile(this.container.base, this.container.base.getCraftOutputs().get(ii).getIndex());
                return;
            }
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(PoseStack poseStack, float f, int x, int y) {
        ...
        for (int i = value; i < endIndex; i++) {
            AutoCraftOutput autoCraftOutput = outputs.get(i);
            ...
            if (stack.isItem()) {
                this.drawItem(x, y, stack.getStack());
            }
            if (stack.isFluid()) {
        ...
        if (this.container.base.index != -1 && container.base.autoCraftSystem != null) {
            ...
            for (int i = startIndex; i < endIndex; i++) {
                if (steps.get(i) instanceof AutoCraftOutput step) {
                    ...
                    if (step.getSameStack().isItem()) {
                        this.drawItem(x, y1, step.getSameStack().getStack());
                    }
    ...
    protected ResourceLocation getTexture() {
        return new ResourceLocation(Constants.MOD_ID, "textures/gui/guiallprocesscrafts.png");
    }
}