...
public class ScreenFluidMonitor<T extends ContainerFluidMonitor> extends ScreenMain<ContainerFluidMonitor> {
    ...
    public void updateTick() {
        ...
        if (canCreate == null) {
            ...
            if (query.startsWith("#") && query.length() >= 2) {
                key = TagKey.create(ForgeRegistries.FLUIDS.getRegistryKey(), ResourceLocation.tryParse(query.substring(1)));
            }
            if (query.startsWith("@") && query.length() >= 2) {
                searchModId = true;
            ...
            if (searchActive) {
                if (viewMode == 0 || viewMode == 1) {
                    for (SameStack s : stacks) {
                        if (ChatFormatting.stripFormatting(com.denfop.utils.ModUtils.cleanComponentString(s.getFluidStack().getDisplayName().getString()).toLowerCase()).startsWith(query)
                                || (searchModId && Registry.FLUID.getKey(s.getFluidStack().getFluid()).getNamespace().startsWith(modId))
    ...
    protected void drawForegroundLayer(PoseStack poseStack, final int par1, final int par2) {
        ...
        if (canCreate == null) {
                ...
                case 0 -> {
                    for (SameStack s : stacks) {
                        ...
                        if (!searchActive || (key != null && stack.getFluid().is(key))
                                || (searchModId && Registry.FLUID.getKey(s.getFluidStack().getFluid()).getNamespace().startsWith(modId))
                                || ChatFormatting.stripFormatting(com.denfop.utils.ModUtils.cleanComponentString(stack.getDisplayName().getString()).toLowerCase()).startsWith(query)) {
                            filtered.add(stack);
                            isCraft.add(false);
                        }
                    }
            ...
            if (decreasing == 2) {
                comparator = Comparator.comparing(ii -> com.denfop.utils.ModUtils.cleanComponentString(finalFiltered1.get(ii).getDisplayName().getString()), collator);
            }
            if (decreasing == 3) {
                comparator = Comparator.comparing(ii -> com.denfop.utils.ModUtils.cleanComponentString(finalFiltered1.get(ii).getDisplayName().getString()), collator.reversed());
            }
                ...
                if (inX && inY) {
                    ...
                    if (!craft) {
                        ...
                        Integer mapForCraft = this.container.base.network.stacksMapCount
                                .getOrDefault(Registry.FLUID.getKey(stack.getFluid()).toString(), new HashMap<>())
            ...
            if (hasCreateGraphScreen) {
                ...
                if (craftGraph != null) {
                    ...
                    if (graphHoveredNodeId != null) {
                        ...
                        if (node != null) {
                            renderTooltip(poseStack, craftGraphRenderer.buildTooltip(node), Optional.empty(), par1, par2);
                        }
                    }
                }
            }
        ...
        switch (this.viewMode) {
            case 0 -> {
                for (SameStack s : stacks) {
                    FluidStack stack = s.getFluidStack();
                    if (!searchActive || (key != null && stack.getFluid().is(key))
                            || (searchModId && Registry.FLUID.getKey(stack.getFluid()).getNamespace().startsWith(modId))
                            || ChatFormatting.stripFormatting(com.denfop.utils.ModUtils.cleanComponentString(stack.getDisplayName().getString()).toLowerCase()).startsWith(query)) {
                        filtered.add(stack);
                        isCraft.add(false);
                    }
            ...
            if (!FluidHandlerFix.hasFluidHandler(carried) || carried.getCount() > 1) {
                if (FluidHandlerFix.hasFluidHandler(carried) && carried.getCount() > 1 && !FluidHandlerFix.getFluidHandler(carried).getFluidInTank(0).isEmpty()) {
                    ...
                    if (canRemove / amount == insertFluid) {
                        new PacketRemoveStack(container.player, insert, container.player.getLevel(), container.base.pos);
                    }
        ...
        if (canCreate == null) {
            ...
            switch (this.viewMode) {
                case 0 -> {
                    for (SameStack s : stacks) {
                        FluidStack stack = s.getFluidStack();
                        if (!searchActive || (searchModId && Registry.FLUID.getKey(stack.getFluid()).getNamespace().startsWith(modId))
            ...
            if (decreasing == 2) {
                comparator = Comparator.comparing(ii -> com.denfop.utils.ModUtils.cleanComponentString(finalFiltered1.get(ii).getFluidStack().getDisplayName().getString()), collator);
            }
            if (decreasing == 3) {
                comparator = Comparator.comparing(ii -> com.denfop.utils.ModUtils.cleanComponentString(finalFiltered1.get(ii).getFluidStack().getDisplayName().getString()), collator.reversed());
            }
            ...
            for (int i = startIndex; i < endIndex; i++) {
                ...
                drawString(poseStack, font, s, 0, 0, 0xFFFFFF);
                poseStack.popPose();
    ...
    protected ResourceLocation getTexture() {
        return switch (this.container.base.sizeMode) {
            default -> new ResourceLocation(Constants.MOD_ID, "textures/gui/guidefaultstoragemonitor.png");
            case 1 -> new ResourceLocation(Constants.MOD_ID, "textures/gui/guibigstoragemonitor.png");
            case 2 -> new ResourceLocation(Constants.MOD_ID, "textures/gui/guismallstoragemonitor.png");
        };
    }
}