...
import com.denfop.containermenu.ContainerMenuSolarPanels;
...
@OnlyIn(Dist.CLIENT)
public class ScreenSolarPanels<T extends ContainerMenuSolarPanels> extends ScreenMain<ContainerMenuSolarPanels> {


    public final BlockEntitySolarPanel tileentity;

    public ScreenSolarPanels(ContainerMenuSolarPanels container) {
        super(container);
        this.componentList.clear();
        this.tileentity = container.tileentity;
        this.imageWidth = 198;
        this.imageHeight = 232;
        this.componentList.add(new ScreenWidget(this, 63, 96, 18, 18,
                new WidgetDefault<>(new ComponentButton(tileentity, 1000, "") {
                    @Override
                    public void ClickEvent() {
                        BlockEntitySolarPanel tileSolarPanel = ((BlockEntitySolarPanel) Minecraft.getInstance().player.getLevel().getBlockEntity(tileentity.pos));
    ...
    protected void drawForegroundLayer(PoseStack poseStack, int mouseX, int mouseY) {
        ...
        this.font.draw(poseStack, formatPanelName, nmPos, 15, 7718655);

        String storageString = Localization.translate("gui.SuperSolarPanel.storage") + ": ";

        new AdvancedTooltipWidget(this, 63, 97, 80, 113).withTooltip(Localization.translate("iu.panel_upgrade.info0")).drawForeground(poseStack, mouseX, mouseY);
        ...
        if (this.tileentity.solarType != 0) {
            ...
            if (this.tileentity.solarType == 1) {
                new AdvancedTooltipWidget(this, 167, 45, 191, 68).withTooltip(ModulesString6).drawForeground(poseStack, mouseX, mouseY);
            }
            if (this.tileentity.solarType == 2) {
                new AdvancedTooltipWidget(this, 167, 45, 191, 68).withTooltip(ModulesString61).drawForeground(poseStack, mouseX, mouseY);
            }
            if (this.tileentity.solarType == 3) {
                new AdvancedTooltipWidget(this, 167, 45, 191, 68).withTooltip(ModulesString62).drawForeground(poseStack, mouseX, mouseY);
            }
            if (this.tileentity.solarType == 4) {
                new AdvancedTooltipWidget(this, 167, 45, 191, 68).withTooltip(ModulesString63).drawForeground(poseStack, mouseX, mouseY);
            }
            if (this.tileentity.solarType == 5) {
                new AdvancedTooltipWidget(this, 167, 45, 191, 68).withTooltip(ModulesString64).drawForeground(poseStack, mouseX, mouseY);
            }
            if (this.tileentity.solarType == 6) {
                new AdvancedTooltipWidget(this, 167, 45, 191, 68).withTooltip(ModulesString65).drawForeground(poseStack, mouseX, mouseY);
            }
            if (this.tileentity.solarType == 7) {
                new AdvancedTooltipWidget(this, 167, 45, 191, 68).withTooltip(ModulesString66).drawForeground(poseStack, mouseX, mouseY);
            }
        ...
        new TooltipWidget(this, 181, 72, 10, 22).withTooltip(temptime).drawForeground(poseStack, mouseX, mouseY);

        new TooltipWidget(this, 28, 51, 84, 17).withTooltip(tooltip).drawForeground(poseStack, mouseX, mouseY);
