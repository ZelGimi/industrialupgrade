...
public class GasSensorScreen extends Screen {
    ...
    private static void beginGuiScissor(int x1, int y1, int x2, int y2) {
        ...
        int scissorY = (int) Math.floor(minecraft.getWindow().getHeight() - y2 * scale);
        int scissorW = (int) Math.ceil((x2 - x1) * scale);
        int scissorH = (int) Math.ceil((y2 - y1) * scale);

        RenderSystem.enableScissor(scissorX, scissorY, Math.max(0, scissorW), Math.max(0, scissorH));
    }

    private static void endGuiScissor() {
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        renderTopControls(poseStack);
        renderFrames(poseStack);

        GasSensorScanState state = snapshot();

        renderMainPanel(poseStack, state, mouseX, mouseY, partialTick);
        renderStatusPanel(poseStack, state);
        renderSelectedPanel(poseStack);

        super.render(poseStack, mouseX, mouseY, partialTick);

        if (!this.hoveredTooltip.isEmpty()) {
            this.renderComponentTooltip(poseStack, this.hoveredTooltip, mouseX, mouseY);
        }
    }
    ...
    private void renderWindow(PoseStack poseStack) {
        ...
        GuiComponent.fill(poseStack, px, py, px + pw, py + ph, WINDOW_BG);
        GuiComponent.fill(poseStack, px + 2, py + 2, px + pw - 2, py + ph - 2, WINDOW_INNER);

        GuiComponent.fill(poseStack, px, py, px + pw, py + 3, 0x88FFFFFF);
        GuiComponent.fill(poseStack, px, py, px + 3, py + ph, BORDER);
        GuiComponent.fill(poseStack, px + pw - 3, py, px + pw, py + ph, BORDER_SOFT);
        GuiComponent.fill(poseStack, px, py + ph - 3, px + pw, py + ph, BORDER_SOFT);
    }
    ...
    private void renderHeader(PoseStack poseStack) {
        ...
        this.itemRenderer.renderAndDecorateItem(this.sensorStack, px + 18, py + 14);
    ...
    private void renderTopControls(PoseStack poseStack) {
        drawSlider(poseStack, topSliderX(), topSliderY(), topSliderW(), MODE_BUTTON_H, this.radiusChunks);
    }

    private void drawSlider(PoseStack poseStack, int x, int y, int w, int h, int value) {
        GuiComponent.fill(poseStack, x, y, x + w, y + h, PANEL_DARK);
        GuiComponent.fill(poseStack, x + 1, y + 1, x + w - 1, y + h - 1, 0xFF23180F);
        GuiComponent.fill(poseStack, x, y, x + w, y + 1, BORDER);
        GuiComponent.fill(poseStack, x, y + h - 1, x + w, y + h, BORDER_SOFT);
        GuiComponent.fill(poseStack, x, y, x + 1, y + h, BORDER);
        GuiComponent.fill(poseStack, x + w - 1, y, x + w, y + h, BORDER_SOFT);

        String text = Component.translatable("iu.gas_sensor.radius", value).getString();
        this.font.draw(poseStack, text, x + 10, y + (h - this.font.lineHeight) / 2 + 1, TEXT_MAIN);
        ...
        GuiComponent.fill(poseStack, trackX, trackY, trackX + trackW, trackY + trackH, 0xFF2E2116);
        ...
        GuiComponent.fill(poseStack, trackX, trackY, knobCenterX, trackY + trackH, 0xFF8E6427);

        int knobX = Mth.clamp(knobCenterX - knobW / 2, trackX, trackX + trackW - knobW);
        int knobY = y + (h - knobH) / 2;

        GuiComponent.fill(poseStack, knobX, knobY, knobX + knobW, knobY + knobH, 0xFFE9E0D2);
        GuiComponent.fill(poseStack, knobX, knobY, knobX + knobW, knobY + 1, 0xFFFFFFFF);
        GuiComponent.fill(poseStack, knobX, knobY + knobH - 1, knobX + knobW, knobY + knobH, 0xFF8B7D68);
        GuiComponent.fill(poseStack, knobX + 2, knobY + 4, knobX + knobW - 2, knobY + knobH - 4, 0xFFBEB19B);
    }

    private void renderFrames(PoseStack poseStack) {
        GuiComponent.fill(poseStack, mainAreaX(), mainAreaY(), mainAreaX() + mainAreaW(), mainAreaY() + mainAreaH(), PANEL_BG);
        GuiComponent.fill(poseStack, mainAreaX() + 1, mainAreaY() + 1, mainAreaX() + mainAreaW() - 1, mainAreaY() + mainAreaH() - 1, PANEL_INNER);

        GuiComponent.fill(poseStack, sideAreaX(), sideAreaY(), sideAreaX() + sideAreaW(), sideAreaY() + statusPanelH(), PANEL_BG);
        GuiComponent.fill(poseStack, sideAreaX() + 1, sideAreaY() + 1, sideAreaX() + sideAreaW() - 1, sideAreaY() + statusPanelH() - 1, PANEL_INNER);

        int selectedY = selectedPanelY();
        GuiComponent.fill(poseStack, sideAreaX(), selectedY, sideAreaX() + sideAreaW(), selectedY + selectedPanelH(), PANEL_BG);
        GuiComponent.fill(poseStack, sideAreaX() + 1, selectedY + 1, sideAreaX() + sideAreaW() - 1, selectedY + selectedPanelH() - 1, PANEL_INNER);
    }

    private void renderMainPanel(PoseStack poseStack, GasSensorScanState state, int mouseX, int mouseY, float partialTick) {
        switch (this.activeMode) {
            case LIST -> renderListMode(poseStack, mouseX, mouseY);
            case DETAIL -> renderDetailMode(poseStack, mouseX, mouseY, partialTick);
            case MAP -> renderMapMode(poseStack, state, mouseX, mouseY);
        }
    }

    private void renderListMode(PoseStack poseStack, int mouseX, int mouseY) {
        int x = listViewportX();
        int y = listViewportY();
        int rowW = listRowsW();
        int h = listViewportH();

        beginGuiScissor(x, y, x + rowW, y + h);

        if (this.visibleVeins.isEmpty()) {
            drawCenteredBoxText(poseStack, x, y, rowW, h, Component.translatable("iu.gas_sensor.list.empty"), TEXT_DIM);
            endGuiScissor();
            renderListScrollbar(poseStack, listScrollbarX(), y, listScrollbarW(), h);
            return;
        }
        ...
        for (GasSensorVeinEntry vein : this.visibleVeins) {
            ...
            drawVeinRow(poseStack, x, drawY, rowW, LIST_ROW_H, vein, selected, hovered);
    ...
    private void drawFluidIcon(PoseStack poseStack, GasSensorVeinEntry vein, int x, int y, int w, int h) {
        ResourceLocation fluidId = ResourceLocation.tryParse(vein.getFluidRegistryName());
        if (fluidId == null) {
            drawFallbackFluidIcon(poseStack, vein, x, y, w, h);
            return;
        }
        ...
        GuiComponent.fill(poseStack, x - 1, y - 1, x + w + 1, y + h + 1, 0xFF4B331F);
        GuiComponent.fill(poseStack, x, y, x + w, y + h, 0xFF1A130C);

        drawTexturedSprite(poseStack, sprite, x + 1, y + 1, w - 2, h - 2, tint);

        GuiComponent.fill(poseStack, x + 1, y + 1, x + w - 1, y + 4, 0x30FFFFFF);
        GuiComponent.fill(poseStack, x + 1, y + h - 2, x + w - 1, y + h - 1, 0x44000000);
    }

    private void drawFallbackFluidIcon(PoseStack poseStack, GasSensorVeinEntry vein, int x, int y, int w, int h) {
        int color = veinColor(vein);

        GuiComponent.fill(poseStack, x - 1, y - 1, x + w + 1, y + h + 1, 0xFF4B331F);
        GuiComponent.fill(poseStack, x, y, x + w, y + h, multiplyColor(color, 0.70F));
        GuiComponent.fill(poseStack, x + 2, y + 2, x + w - 2, y + 10, lighten(color, 0.28F));
        GuiComponent.fill(poseStack, x + 2, y + 11, x + w - 2, y + h - 2, color);
    }

    private void drawTexturedSprite(PoseStack poseStack, TextureAtlasSprite sprite, int x, int y, int w, int h, int color) {
        ...
        buffer.vertex(matrix, x, y + h, 0.0F).uv(sprite.getU0(), sprite.getV1()).color(r, g, b, a).endVertex();
        buffer.vertex(matrix, x + w, y + h, 0.0F).uv(sprite.getU1(), sprite.getV1()).color(r, g, b, a).endVertex();
        buffer.vertex(matrix, x + w, y, 0.0F).uv(sprite.getU1(), sprite.getV0()).color(r, g, b, a).endVertex();
        buffer.vertex(matrix, x, y, 0.0F).uv(sprite.getU0(), sprite.getV0()).color(r, g, b, a).endVertex();

        BufferUploader.drawWithShader(buffer.end());
        RenderSystem.disableBlend();
    }

    private void drawVeinRow(PoseStack poseStack, int x, int y, int w, int h, GasSensorVeinEntry vein, boolean selected, boolean hovered) {
        int fill = selected ? ROW_BG_SELECTED : hovered ? ROW_BG_HOVER : ROW_BG;
        GuiComponent.fill(poseStack, x, y, x + w, y + h, fill);
        GuiComponent.fill(poseStack, x + 1, y + 1, x + w - 1, y + h - 1, 0xFF21170F);
        GuiComponent.fill(poseStack, x, y, x + w, y + 1, selected ? ACCENT : ROW_LINE);
        GuiComponent.fill(poseStack, x, y + h - 1, x + w, y + h, selected ? ACCENT_SOFT : ROW_LINE);
        GuiComponent.fill(poseStack, x, y, x + 1, y + h, selected ? ACCENT : ROW_LINE);
        GuiComponent.fill(poseStack, x + w - 1, y, x + w, y + h, selected ? ACCENT_SOFT : ROW_LINE);

        int iconX = x + 12;
        int iconY = y + 16;
        drawFluidIcon(poseStack, vein, iconX, iconY, 30, 30);
        ...
        this.font.draw(poseStack, name, textX, y + 16, TEXT_MAIN);
        this.font.draw(poseStack, coord, textX, y + 40, TEXT_DIM);

        this.font.draw(poseStack, volume, rightX, y + 14, TEXT_HIGHLIGHT);
        this.font.draw(poseStack, distance, rightX, y + 40, TEXT_DIM);

        if (selected || hovered) {
            GuiComponent.fill(poseStack, x + 4, y + h - 6, x + w - 4, y + h - 2, selected ? ACCENT : ACCENT_DARK);
        }
    }

    private void renderListScrollbar(PoseStack poseStack, int x, int y, int w, int h) {
        GuiComponent.fill(poseStack, x, y, x + w, y + h, 0xFF1B130D);
        GuiComponent.fill(poseStack, x, y, x + w, y + 1, BORDER);
        GuiComponent.fill(poseStack, x, y + h - 1, x + w, y + h, BORDER_SOFT);

        int maxScroll = listMaxScroll();
        if (maxScroll <= 0) {
            GuiComponent.fill(poseStack, x + 1, y + 1, x + w - 1, y + h - 1, 0xFF5E4325);
            return;
        }
        ...
        GuiComponent.fill(poseStack, x + 1, thumbY, x + w - 1, thumbY + thumbH, ACCENT_SOFT);
        GuiComponent.fill(poseStack, x + 1, thumbY, x + w - 1, thumbY + 2, 0xAAFFFFFF);
    }

    private void renderDetailMode(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        ...
        GuiComponent.fill(poseStack, innerX, innerY, innerX + innerW, innerY + previewH, 0xFF0D0906);
        GuiComponent.fill(poseStack, innerX + 1, innerY + 1, innerX + innerW - 1, innerY + previewH - 1, 0xFF140E09);

        GuiComponent.fill(poseStack, innerX, infoY, innerX + innerW, infoY + infoH, 0xFF120D08);
        GuiComponent.fill(poseStack, innerX + 1, infoY + 1, innerX + innerW - 1, infoY + infoH - 1, 0xFF1A130C);

        beginGuiScissor(innerX + 2, innerY + 2, innerX + innerW - 2, innerY + previewH - 2);
        renderDetailPreview(poseStack, innerX, innerY, innerW, previewH);
        endGuiScissor();

        renderDetailInfoBox(poseStack, infoY, innerX, innerW, infoH);
    }

    private void renderDetailPreview(PoseStack poseStack, int x, int y, int w, int h) {
        GasSensorVeinEntry vein = selectedVein().orElse(null);
        if (vein == null) {
            drawCenteredBoxText(poseStack, x, y, w, h, Component.translatable("iu.gas_sensor.selected.none.body"), TEXT_DIM);
            return;
        }

        List<GasSensorVeinEntry.PreviewCell> cells = vein.getPreviewCells();
        if (cells.isEmpty()) {
            drawCenteredBoxText(poseStack, x, y, w, h, Component.translatable("iu.gas_sensor.detail.no_preview"), TEXT_DIM);
            return;
        }

        FluidRenderData fluid = resolveFluidRenderData(vein);
        if (fluid == null) {
            renderFallbackDetailPreview(poseStack, x, y, w, h, vein, cells);
            return;
        }

        renderFluidDetailPreview(poseStack, x, y, w, h, vein, cells, fluid);
    }
    ...
    private void renderFluidDetailPreview(
        ...
        poseStack.mulPose(Vector3f.YP.rotationDegrees(this.detailYaw));
        poseStack.mulPose(Vector3f.XP.rotationDegrees(-pitchDegrees));
        poseStack.translate(
                -((minX + maxX + 1) / 2.0F),
                -(minY + spanY * 0.45F),
                -((minZ + maxZ + 1) / 2.0F)
    ...
    private void renderFallbackDetailPreview(
        ...
        for (ProjectedCell cell : projected) {
            ...
            GuiComponent.fill(poseStack, sx, sy, sx + size, sy + size, color);
            GuiComponent.fill(poseStack, sx + 1, sy + 1, sx + size - 1, sy + Math.max(2, size / 3), top);
            GuiComponent.fill(poseStack, sx, sy, sx + size, sy + 1, 0x60FFFFFF);
            GuiComponent.fill(poseStack, sx, sy + size - 1, sx + size, sy + size, 0x44000000);
        }
    }
    ...
    private void drawFluidHorizontalQuad(
        ...
        Matrix4f matrix = poseStack.last().pose();
        builder.vertex(matrix, x0, y0, z0).color(r, g, b, a).uv(u0, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
        builder.vertex(matrix, x0, y1, z1).color(r, g, b, a).uv(u0, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
        builder.vertex(matrix, x1, y1, z1).color(r, g, b, a).uv(u1, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
        builder.vertex(matrix, x1, y0, z0).color(r, g, b, a).uv(u1, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
    }
    ...
    private void drawFluidVerticalQuadZ(
        ...
        if (north) {
            builder.vertex(matrix, x0, y0, z0).color(r, g, b, a).uv(u0, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x0, y1, z0).color(r, g, b, a).uv(u0, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x1, y1, z1).color(r, g, b, a).uv(u1, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x1, y0, z1).color(r, g, b, a).uv(u1, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
        } else {
            builder.vertex(matrix, x1, y0, z1).color(r, g, b, a).uv(u0, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x1, y1, z1).color(r, g, b, a).uv(u0, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x0, y1, z0).color(r, g, b, a).uv(u1, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x0, y0, z0).color(r, g, b, a).uv(u1, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
        }
    }
    ...
    private void drawFluidVerticalQuadX(
        ...
        if (west) {
            builder.vertex(matrix, x, y0, z1).color(r, g, b, a).uv(u0, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x, y1, z1).color(r, g, b, a).uv(u0, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x, y1, z0).color(r, g, b, a).uv(u1, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x, y0, z0).color(r, g, b, a).uv(u1, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
        } else {
            builder.vertex(matrix, x, y0, z0).color(r, g, b, a).uv(u0, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x, y1, z0).color(r, g, b, a).uv(u0, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x, y1, z1).color(r, g, b, a).uv(u1, v0).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
            builder.vertex(matrix, x, y0, z1).color(r, g, b, a).uv(u1, v1).uv2(packedLight).normal(0.0F, 1.0F, 0.0F).endVertex();
        }
    }
    ...
    private FluidRenderData resolveFluidRenderData(GasSensorVeinEntry vein) {
        ...
        ResourceLocation flowTex = ext.getFlowingTexture(stack);
        if (stillTex == null || flowTex == null) {
            return null;
        }

        TextureAtlasSprite still = Minecraft.getInstance().getTextureAtlas(TextureAtlas.LOCATION_BLOCKS).apply(stillTex);
        TextureAtlasSprite flow = Minecraft.getInstance().getTextureAtlas(TextureAtlas.LOCATION_BLOCKS).apply(flowTex);

        int tint = 0xBFFFFFFF;
        try {
            tint = ext.getTintColor(stack);
            if ((tint >>> 24) == 0) {
    ...
    private void renderDetailInfoBox(PoseStack poseStack, int y, int x, int w, int h) {
        int contentX = x + 12;
        int contentW = w - 24;
        int maxY = y + h - 10;
        int yy = y + 10;

        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.detail.window.title"), contentX, yy, contentW, TEXT_MAIN, maxY);
        yy += 4;
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.detail.controls"), contentX, yy, contentW, TEXT_DIM, maxY);

        GasSensorVeinEntry vein = selectedVein().orElse(null);
        if (vein == null) {
            drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.none.body"), contentX, yy + 6, contentW, TEXT_MUTED, maxY);
            return;
        }

        yy += 6;
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.name", veinName(vein)), contentX, yy, contentW, TEXT_MAIN, maxY);
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.type", veinType(vein)), contentX, yy, contentW, TEXT_DIM, maxY);
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.volume", formatBuckets(veinBuckets(vein))), contentX, yy, contentW, TEXT_HIGHLIGHT, maxY);
        drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.preview_cells", vein.getPreviewCellCount()), contentX, yy, contentW, TEXT_HIGHLIGHT, maxY);
    }

    private void renderMapMode(PoseStack poseStack, GasSensorScanState state, int mouseX, int mouseY) {
        ...
        GuiComponent.fill(poseStack, x, y, x + w, y + h, 0xFF0B0805);
        GuiComponent.fill(poseStack, x + 1, y + 1, x + w - 1, y + h - 1, 0xFF110C08);

        int legendX = mapLegendX();
        int legendY = mapLegendY();
        int legendW = mapLegendW();

        GuiComponent.fill(poseStack, legendX, legendY, legendX + legendW, legendY + MAP_LEGEND_H, 0xAA140E09);
        GuiComponent.fill(poseStack, legendX, legendY, legendX + legendW, legendY + 1, BORDER);
        GuiComponent.fill(poseStack, legendX, legendY + MAP_LEGEND_H - 1, legendX + legendW, legendY + MAP_LEGEND_H, BORDER_SOFT);
        ...
        legendTextY = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.map.legend.player"), textX, legendTextY, textW, TEXT_MAIN, maxY);
        legendTextY = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.map.legend.veins"), textX, legendTextY + 2, textW, TEXT_DIM, maxY);
        drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.map.controls"), textX, legendTextY + 4, textW, TEXT_MUTED, maxY);
        ...
        GuiComponent.fill(poseStack, viewX, viewY, viewX + viewW, viewY + viewH, 0xFF0E0A06);
        GuiComponent.fill(poseStack, viewX + 1, viewY + 1, viewX + viewW - 1, viewY + viewH - 1, 0xFF150F09);

        beginGuiScissor(viewX, viewY, viewX + viewW, viewY + viewH);
        renderMapGrid(poseStack, state, viewX, viewY, viewW, viewH, mouseX, mouseY);
        endGuiScissor();
    }

    private void renderMapGrid(PoseStack poseStack, GasSensorScanState state, int x, int y, int w, int h, int mouseX, int mouseY) {
        ...
        for (int gz = -radius; gz <= radius; gz++) {
            for (int gx = -radius; gx <= radius; gx++) {
                ...
                GuiComponent.fill(poseStack, ix, iy, ix2, iy2, fill);

                int line = (gx == 0 || gz == 0) ? MAP_GRID_BOLD : MAP_GRID;
                GuiComponent.fill(poseStack, ix, iy, ix2, iy + 1, line);
                GuiComponent.fill(poseStack, ix, iy2 - 1, ix2, iy2, line);
                GuiComponent.fill(poseStack, ix, iy, ix + 1, iy2, line);
                GuiComponent.fill(poseStack, ix2 - 1, iy, ix2, iy2, line);
            }
        }
        ...
        if (state != null) {
            ...
            GuiComponent.fill(poseStack, px, py, px + playerSize, py + playerSize, 0xFFF0F0F0);
        }
        ...
        for (GasSensorVeinEntry vein : this.visibleVeins) {
            ...
            GuiComponent.fill(poseStack, left, top, right, top + 1, 0x66FFFFFF);
    ...
    private void renderStatusPanel(PoseStack poseStack, GasSensorScanState state) {
        int x = sideAreaX() + 18;
        int y = sideAreaY() + 18;
        int w = sideAreaW() - 36;
        int h = statusPanelH() - 36;

        GuiComponent.fill(poseStack, x, y, x + w, y + h, 0x660F0B07);

        if (state != null && !state.isSupportedDimension()) {
            int maxY = y + h - 8;
            int yy = y + 20;
            yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.status.unsupported"), x + 16, yy, w - 32, TEXT_MAIN, maxY);
            drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.status.unsupported_hint"), x + 16, yy + 6, w - 32, TEXT_DIM, maxY);
            return;
        }
        ...
        drawCircularProgress(poseStack, ringCx, ringCy, ringRadius, progress, scanning ? ACCENT : ACCENT_GREEN);

        String percent = Math.round(progress * 100.0F) + "%";
        GuiComponent.drawCenteredString(poseStack, this.font, percent, ringCx, ringCy - 4, TEXT_MAIN);
        ...
        yy += 6;
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.status.progress", percent), textX, yy, textW, TEXT_DIM, maxY);
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.status.chunk", snapshotChunkX(state), snapshotChunkZ(state)), textX, yy, textW, TEXT_DIM, maxY);
        drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.status.found", snapshotFoundCount(state)), textX, yy, textW, TEXT_HIGHLIGHT, maxY);
    }

    private void drawCircularProgress(PoseStack poseStack, int cx, int cy, int radius, float progress, int color) {
        int dots = 56;
        for (int i = 0; i < dots; i++) {
            ...
            int size = active ? 4 : 3;
            GuiComponent.fill(poseStack, dx - size / 2, dy - size / 2, dx + size / 2, dy + size / 2, dotColor);
        }
    }

    private void renderSelectedPanel(PoseStack poseStack) {
        int x = sideAreaX() + 18;
        ...
        GuiComponent.fill(poseStack, x, y, x + w, y + h, 0x660F0B07);
        this.font.draw(poseStack, Component.translatable("iu.gas_sensor.selected.title"), x + 14, y + 14, TEXT_MAIN);
        ...
        GasSensorVeinEntry vein = selectedVein().orElse(null);
        if (vein == null) {
            drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.none.body"), contentX, yy, contentW, TEXT_DIM, maxY);
            return;
        }

        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.name", veinName(vein)), contentX, yy, contentW, TEXT_MAIN, maxY);
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.type", veinType(vein)), contentX, yy, contentW, TEXT_DIM, maxY);
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.coords", vein.getBlockX(), vein.getBlockY(), vein.getBlockZ()), contentX, yy, contentW, TEXT_DIM, maxY);
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.chunk", vein.getChunkX(), vein.getChunkZ()), contentX, yy, contentW, TEXT_DIM, maxY);
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.volume", formatBuckets(veinBuckets(vein))), contentX, yy, contentW, TEXT_HIGHLIGHT, maxY);
        yy = drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.distance", formatDistance(veinDistanceMeters(vein))), contentX, yy, contentW, TEXT_DIM, maxY);
        drawWrappedComponent(poseStack, Component.translatable("iu.gas_sensor.selected.preview_cells", vein.getPreviewCellCount()), contentX, yy, contentW, TEXT_HIGHLIGHT, maxY);
    }
    ...
    private void requestServerScan() {
        ...
        new PacketGasSensorScanRequest(this.radiusChunks);
        resetMapView();
    }
    ...
    private int drawWrappedComponent(PoseStack poseStack, Component text, int x, int y, int width, int color, int maxY) {
        ...
        for (FormattedCharSequence line : lines) {
            ...
            this.font.draw(poseStack, line, x, yy, color);
    ...
    private void drawCenteredBoxText(PoseStack poseStack, int x, int y, int w, int h, Component text, int color) {
        ...
        for (FormattedCharSequence line : lines) {
            this.font.draw(poseStack, line, x + Math.max(0, (w - this.font.width(line)) / 2), yy, color);
            yy += this.font.lineHeight + 2;
        }
    }
    ...
    private static class OrangeButton extends AbstractWidget {
        ...
        public void renderButton(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
            ...
            GuiComponent.fill(poseStack, this.x, this.y, this.x + this.width, this.y + this.height, outer);
            GuiComponent.fill(poseStack, this.x + 1, this.y + 1, this.x + this.width - 1, this.y + this.height - 1, inner);
            GuiComponent.fill(poseStack, this.x + 2, this.y + 2, this.x + this.width - 2, this.y + this.height / 2, top);
            GuiComponent.fill(poseStack, this.x + 4, this.y + this.height - 6, this.x + this.width - 4, this.y + this.height - 2, this.accent);

            if (hovered) {
                GuiComponent.fill(poseStack, this.x + 2, this.y + 2, this.x + this.width - 2, this.y + 4, 0x30FFFFFF);
            }
            if (selected) {
                GuiComponent.fill(poseStack, this.x + 2, this.y + 2, this.x + this.width - 2, this.y + 4, 0x55FFFFFF);
            }

            Font font = Minecraft.getInstance().font;
            int textX = this.x + this.width / 2;
            int textY = this.y + (this.height - font.lineHeight) / 2 + 1;
            GuiComponent.drawCenteredString(poseStack, font, this.getMessage(), textX, textY, 0xFFF3E7D1);
        }
    ...
    private static class OrangeEditBox extends EditBox {
        ...
        @Override
        public void renderButton(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
            GuiComponent.fill(poseStack, this.x, this.y, this.x + this.width, this.y + this.height, 0xFF1A120B);
            GuiComponent.fill(poseStack, this.x + 1, this.y + 1, this.x + this.width - 1, this.y + this.height - 1, 0xFF22170E);

            GuiComponent.fill(poseStack, this.x, this.y, this.x + this.width, this.y + 1, 0xFF8A5A22);
            GuiComponent.fill(poseStack, this.x, this.y + this.height - 1, this.x + this.width, this.y + this.height, 0xFF4B3116);
            GuiComponent.fill(poseStack, this.x, this.y, this.x + 1, this.y + this.height, 0xFF8A5A22);
            GuiComponent.fill(poseStack, this.x + this.width - 1, this.y, this.x + this.width, this.y + this.height, 0xFF4B3116);
            ...
            poseStack.pushPose();
            poseStack.translate(textInset, textInset, 0.0F);
