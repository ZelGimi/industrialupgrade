...
public class ScreenGeothermalGenerator<T extends ContainerMenuGeothermalgenerator> extends ScreenMain<ContainerMenuGeothermalgenerator> {

    public ScreenGeothermalGenerator(ContainerMenuGeothermalgenerator guiContainer) {
        super(guiContainer);
        this.componentList.clear();
        this.addComponent(new ScreenWidget(this, 110, (this.imageHeight - 80) / 2, EnumTypeComponent.QUANTUM_ENERGY_WEIGHT,
                new WidgetDefault<>(this.container.base.getEnergy())
        ));
        this.addWidget((new FluidDefaultWidget(this, this.imageWidth / 2 - 10, 20,
                ((BlockEntityGeothermalGenerator) guiContainer.base).getFluidTank().getFluid()
        ) {
            @Override
            public void drawBackground(GuiGraphics poseStack, final int mouseX, final int mouseY) {
                bindCommonTexture();
                FluidStack fs = ((BlockEntityGeothermalGenerator) guiContainer.base).getFluidTank().getFluid();
