...
public class ScreenIndustrialUpgrade<T extends ContainerMenuBase<? extends CustomWorldContainer>> extends AbstractContainerScreen<T> implements MenuAccess<T> {
            ...
            if (slot instanceof SlotInvSlot) {
                ...
                if (this.isHovering(slot, mouseX, mouseY)) {
                    ...
                    if (((SlotInvSlot) slot).inventory.hasItemList() && Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
                        renderItemTooltipGrid(graphics, ((SlotInvSlot) slot).inventory.getStacks(((SlotInvSlot) slot).index), guiLeft + j2, guiTop + k2);
                    }
                }
            }
        }
    ...
    public void handleUpgradeTooltip(int n, int n2) {
    ...
    public boolean mouseScrolled(double d, double d2, double d3) {
        ...
        for (Renderable button : listButton) {
            if (button instanceof VerticalSliderListWidget) {
                VerticalSliderListWidget slider = (VerticalSliderListWidget) button;
            ...
            if (button instanceof SliderWidget) {
                SliderWidget slider = (SliderWidget) button;
                if (scrollDirection != ScrollDirection.stopped) {

                    slider.handleMouseWheel(scrollDirection, (int) (d), (int) (d2));
                }
            }
            if (button instanceof VerticalSliderWidget) {
                VerticalSliderWidget slider = (VerticalSliderWidget) button;
                if (scrollDirection != ScrollDirection.stopped) {
                    slider.handleMouseWheel(scrollDirection, (int) (d), (int) (d2));
                }
            }
    ...
    protected void mouseClicked(int i, int j, int k) {
        for (Renderable widget : this.renderables) {
            if (widget instanceof GuiEventListener && !children().contains(widget)) {
    ...
    public boolean mouseClicked(double d, double d2, int n) {
        ...
        for (ScreenWidget element : this.elements) {
            if (element.visible()) {
                element.onMouseClick((int) d, (int) d2, mouseButton);
            }
        }
        d += this.leftPos;
        d2 += topPos;
    ...
    public void drawTooltip(GuiGraphics poseStack, int x, int y, ItemStack stack, List<String> strings) {
        ...
        List<Component> tooltipComponents = new ArrayList<>();
        List<Component> components = stack.getTooltipLines(container.player, TooltipFlag.NORMAL);
