...
public class ScreenPreCraft<T extends ContainerPreCraft> extends ScreenMain<ContainerPreCraft> {
    ...
    protected void mouseClicked(int ii, int j, int k) {
        ...
        for (int i = 0; i < 10; i++) {
            if (this.countField[i].mouseClicked(i, j, k)) {
                countField[i].setFocus(true);
                return;
            } else {
                countField[i].setFocus(false);
            }
        }
        for (int i = 0; i < 10; i++) {
            int x1 = 8 + (i / 5) * 116;
    ...
    protected ResourceLocation getTexture() {
        return new ResourceLocation(Constants.MOD_ID, "textures/gui/guiprecraft.png");
