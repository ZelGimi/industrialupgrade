...
public class ScreenCable2<T extends ContainerMenuCable> extends ScreenMain<ContainerMenuCable> {
    ...
    public ScreenCable2(ContainerMenuCable guiContainer) {
        super(guiContainer);
    ...
    @Override
    protected void drawGuiContainerBackgroundLayer(PoseStack guiGraphics, float partialTicks, int mouseX, int mouseY) {
        ...
        CablePreviewStateFactory.PreparedPreview preparedPreview = CablePreviewStateFactory.prepare(this.container.base.getBlockState(), displayBlacklist);
        ...
        enableScissor(previewAbsX, previewAbsY, previewAbsX + PREVIEW_W, previewAbsY + PREVIEW_H);
        ...
        this.previewRenderer.renderWithoutOutline(
                guiGraphics,
                previewAbsX,
                previewAbsY,
                PREVIEW_W,
    ...
    private void drawPanels(PoseStack guiGraphics, int previewAbsX, int previewAbsY) {
        fill(guiGraphics,
                this.guiLeft + 4,
                this.guiTop + 4,
        ...
        fill(guiGraphics, previewAbsX - 2, previewAbsY - 2, previewAbsX + PREVIEW_W + 2, previewAbsY + PREVIEW_H + 2, 0xC0080A10);
        fill(guiGraphics, previewAbsX - 1, previewAbsY - 1, previewAbsX + PREVIEW_W + 1, previewAbsY + PREVIEW_H + 1, 0xFF2B3245);
        fill(guiGraphics, previewAbsX, previewAbsY, previewAbsX + PREVIEW_W, previewAbsY + PREVIEW_H, 0xFF0F1726);
    ...
    private void drawHints(PoseStack guiGraphics) {
        int panelX = this.guiLeft + CONTENT_X;
        int panelY = this.guiTop + HINT_PANEL_Y;

        int textX = panelX + 4;
        int textY = panelY + 4;
        int maxWidth = CONTENT_W - 8;
        int maxBottom = panelY + HINT_PANEL_H - 3;
    ...
    private void drawWrappedTextBlock(PoseStack guiGraphics,
                                      Component text,
                                      int x,
                                      int y,
                                      int maxWidth,
        ...
        for (FormattedCharSequence line : lines) {
            ...
            drawString(guiGraphics, this.font, line, x, currentY, color);
            currentY += lineHeight;
        }
    ...
    private void drawDirectionBadges(PoseStack guiGraphics, EnumSet<Direction> displayBlacklist) {
        int previewAbsX = this.guiLeft + PREVIEW_X;
        int previewAbsY = this.guiTop + PREVIEW_Y;
        ...
        for (Direction direction : Direction.values()) {
            ...
            guiGraphics.translate(0.0F, 0.0F, 500.0F);
            fill(guiGraphics, x, y, x + badgeW, y + badgeH, outer);
            fill(guiGraphics, x + 1, y + 1, x + badgeW - 1, y + badgeH - 1, inner);
            drawCenteredString(guiGraphics, this.font, shortDirection(direction), x + badgeW / 2, y + 2, textColor);
            guiGraphics.popPose();
        }
    }
    ...
    private OverlayButtonBounds buildOverlayButton(EnumSet<Direction> displayBlacklist, boolean keepSticky) {
        ...
        Component label = tr("iu.cable.preview.button.open");
    ...
    private void drawOverlayButton(PoseStack guiGraphics, int mouseX, int mouseY) {
        ...
        fill(guiGraphics, x - 2, y - 2, x + w + 2, y + h + 2, shadow);
        fill(guiGraphics, x - 1, y - 1, x + w + 1, y + h + 1, outer);
        fill(guiGraphics, x, y, x + w, y + h, border2);
        fill(guiGraphics, x + 1, y + 1, x + w - 1, y + h - 1, inner);
    ...
    private void renderHoverTooltip(PoseStack guiGraphics, int mouseX, int mouseY) {
        if (this.overlayButtonBounds != null && this.overlayButtonBounds.contains(mouseX, mouseY)) {
            return;
        }
    ...
    private void toggleDirection(Direction direction) {
        ...
        ((BlockEntityItemPipes) this.container.base).facingSide = direction;
        new PacketUpdateServerTile(this.container.base, 100 + direction.ordinal());
