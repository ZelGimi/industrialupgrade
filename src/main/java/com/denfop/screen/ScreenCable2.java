...
public class ScreenCable2<T extends ContainerMenuCable> extends ScreenMain<ContainerMenuCable> {
    ...
    @Override
    public boolean mouseScrolled(double mouseX, double mouseY, double delta1, double delta) {
        if (isInsidePreview(mouseX, mouseY)) {
            this.zoom = Mth.clamp(this.zoom + (float) delta * 0.08F, 0.72F, 1.80F);
            return true;
        }
        return super.mouseScrolled(mouseX, mouseY, delta1, delta);
    }
    ...
    private void drawWrappedTextBlock(GuiGraphics guiGraphics,
                                      Component text,
                                      int x,
                                      int y,
                                      int maxWidth,
