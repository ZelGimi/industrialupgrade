...
public class ScreenPlanetaryTranslocator extends Screen {
    ...
    private float renderSystemBlock(
            final GuiGraphics graphics,
            final SystemNode systemNode,
            final int x,
            final float y,
            final int width,
            final int mouseX,
    ...
    private float renderBodyBlockClipped(
            final GuiGraphics graphics,
            final BodyNode bodyNode,
            final int x,
            final float y,
            final int width,
            final int mouseX,
            ...
            final float visibleHeightBudget
    ) {
    ...
    private void renderRightInfo(final GuiGraphics graphics, final int mouseX, final int mouseY) {
        ...
        if (selected instanceof SystemNode systemNode) {
            ...
            renderStarPreview3D(
                    graphics,
                    systemNode.star,
                    left + PREVIEW_BOX_X + PREVIEW_BOX_W / 2,
                    top + PREVIEW_BOX_Y + PREVIEW_BOX_H / 2,
                    30.0F,
                    18.0F
            );
            ...
            renderBodyPreview3D(
                    ...
                    left + PREVIEW_BOX_X + PREVIEW_BOX_W / 2,
                    top + PREVIEW_BOX_Y + PREVIEW_BOX_H / 2,
                    resolvePreviewRadius(bodyNode.body),
                    18.0F
            );
        }
    ...
    private List<InfoLine> buildInfoLines() {
        ...
        if (selected instanceof BodyNode bodyNode) {
            ...
            if (body instanceof IPlanet planet) {
                lines.add(new InfoLine(
                        fallback("iu.space.tp.temperature") + ": " + planet.getTemperature() + "C°",
                        0xFFFFFF
                ));
                lines.add(new InfoLine(
                        fallback("iu.space.tp.oxygen") + ": " + yesNo(planet.hasOxygen()),
                        planet.hasOxygen() ? 0x76FF76 : 0xFFB0B0
                ));
                if (!planet.getSatelliteList().isEmpty()) {
                    lines.add(new InfoLine(
                            fallback("iu.space.tp.moons_hint"),
                            0xBEEAFF
                    ));
                }
            } else if (body instanceof ISatellite satellite) {
                lines.add(new InfoLine(
                        fallback("iu.space.tp.temperature") + ": " + satellite.getTemperature() + "C°",
                        0xFFFFFF
                ));
                lines.add(new InfoLine(
                        fallback("iu.space.tp.oxygen") + ": " + yesNo(satellite.hasOxygen()),
                        satellite.hasOxygen() ? 0x76FF76 : 0xFFB0B0
                ));
                lines.add(new InfoLine(
                        fallback("iu.space.tp.parent_planet") + ": " + fallback("iu.body." + satellite.getPlanet().getName()),
                        0x8EDFFF
                ));
            } else if (body instanceof IAsteroid asteroid) {
                lines.add(new InfoLine(
                        fallback("iu.space.tp.temperature") + ": " + asteroid.getTemperature() + "C°",
                        0xFFFFFF
                ));
                lines.add(new InfoLine(
                        fallback("iu.space.tp.asteroid_info"),
                        0xDDEAFF
                ));
            }
            ...
            lines.add(new InfoLine(
                    fallback("iu.space.tp.max_stay") + ": " + formatTicks((long) (maxStaySeconds * 20.0D)),
                    0xFFD560
            ));
            lines.add(new InfoLine(
                    fallback("iu.space.tp.energy_drain") + ": " + ModUtils.getString(drainPerSecond) + " EF/s",
                    0xEAF9FF
            ));
            lines.add(new InfoLine(
                    fallback("iu.space.tp.teleport_cost") + ": " + ModUtils.getString(teleportCost) + " EF",
                    0x8EDFFF
            ));
    ...
    private BodyHitTestResult hitTestBodyBlock(
            final int mouseX,
            final int mouseY,
            final SystemNode systemNode,
            final BodyNode bodyNode,
    ...
    private boolean isMouseOverClipped(
            final int mouseX,
            final int mouseY,
            final int x,
            final int y,
    ...
    private void pushScissor(final int x1, final int y1, final int x2, final int y2) {
        ...
        if (!scissorStack.isEmpty()) {
            ...
            next = new ScissorRect(
                    Math.max(prev.x1, next.x1),
                    Math.max(prev.y1, next.y1),
                    Math.min(prev.x2, next.x2),
                    Math.min(prev.y2, next.y2)
            );
        }
    ...
    private void renderStarPreview3D(
            final GuiGraphics graphics,
            final IStar star,
            final float x,
            final float y,
    ...
    private void renderBodyPreview3D(
            final GuiGraphics graphics,
            final IBody body,
            final float x,
            final float y,
    ...
    private long estimateFieldTicks(final double charge) {
        ...
        return SpaceTeleportEnergyLogic.estimateRemainingTicks(
                ...
                body,
                maxCharge,
                SpaceTeleportController.CRITICAL_CHARGE
        );
    }
