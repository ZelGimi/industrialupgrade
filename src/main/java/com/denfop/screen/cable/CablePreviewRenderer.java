...
public class CablePreviewRenderer {
    ...
    public void render(PoseStack poseStack,
        ...
        poseStack.pushPose();
        applyTransform(poseStack, x, y, width, height, yaw, pitch, zoom);

        renderNeighborBlocks(poseStack, bufferSource, neighborStates);
        renderBlockModel(poseStack, bufferSource, preview);
        renderOutlineState(poseStack, bufferSource, preview, blacklist, hovered);
    ...
    public void renderWithoutOutline(PoseStack poseStack,
                                     int x,
                                     int y,
                                     int width,
                                     int height,
    ...
    protected void renderBlockModel(PoseStack poseStack,
    ...
    private void renderNeighborBlocks(PoseStack poseStack,
                                      MultiBufferSource.BufferSource bufferSource,
                                      EnumMap<Direction, BlockState> neighborStates) {
        Minecraft minecraft = Minecraft.getInstance();
    ...
    private void renderOutlineState(PoseStack poseStack,
                                    MultiBufferSource.BufferSource bufferSource,
                                    CablePreviewStateFactory.PreparedPreview preview,
                                    EnumSet<Direction> blacklist,
                                    CablePreviewHitResult hovered) {
    ...
    public CablePreviewHitResult pick(int mouseX,
        ...
        Matrix4f inverse = modelToScreen.copy();

        if (!inverse.invert()) {
            return null;
        }

        Vector4f near = new Vector4f(mouseX, mouseY, -1000.0F, 1.0F);
        Vector4f far = new Vector4f(mouseX, mouseY, 1000.0F, 1.0F);

        near.transform(inverse);
        far.transform(inverse);
    ...
    public ScreenProjection projectPoint(Vec3 point,
        ...
        Vector4f vec = new Vector4f((float) point.x, (float) point.y, (float) point.z, 1.0F);
    ...
    private Matrix4f buildMatrix(int x, int y, int width, int height, float yaw, float pitch, float zoom) {
        ...
        matrix.multiply(Matrix4f.createTranslateMatrix(centerX, centerY, MODEL_Z));
        matrix.multiply(Matrix4f.createScaleMatrix(scale, -scale, scale));
        matrix.multiply(Vector3f.XP.rotationDegrees(pitch));
        matrix.multiply(Vector3f.YP.rotationDegrees(yaw));
        matrix.multiply(Matrix4f.createTranslateMatrix(-0.5F, -0.5F, -0.5F));
