...
public class ScreenRelocator<T extends ContainerMenuBase<ItemStackRelocator>> extends ScreenMain<ContainerMenuBase<ItemStackRelocator>> {
    ...
    protected void drawForegroundLayer(PoseStack poseStack, final int par1, final int par2) {
        ...
        for (int i = 4 * value; i < Math.min(4 + 4 * value, pointList.size()); i++) {
            ...
            drawString(poseStack, truncatedName, 24, 22 + 15 * (i % 4), ModUtils.convertRGBcolorToInt(255, 255, 255));
            new TooltipWidget(this, 7, 22 + 20 * (i % 4), 160, 14).withTooltip(point.getPos().toString().substring(8)).drawForeground(
                    poseStack, par1,
                    par2
            );
        }
    ...
    protected void drawGuiContainerBackgroundLayer(PoseStack poseStack, final float partialTicks, final int mouseX, final int mouseY) {
        ...
        drawString(poseStack, Minecraft.getInstance().font, name, textX, textY, ModUtils.convertRGBcolorToInt(255, 255, 255));
