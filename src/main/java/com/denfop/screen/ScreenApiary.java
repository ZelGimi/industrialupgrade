...
@OnlyIn(Dist.CLIENT)
public class ScreenApiary<T extends ContainerMenuApiary> extends ScreenMain<ContainerMenuApiary> {
    ...
    public ScreenApiary(ContainerMenuApiary container1) {
        ...
        this.addWidget(new AdvancedTooltipWidget(this, 10, 21, 21, 69).withTooltip(() -> ModUtils.getString(container1.base.food)));
        this.addWidget(new AdvancedTooltipWidget(this, 154, 21, 165, 69).withTooltip(() -> ModUtils.getString(container1.base.royalJelly)));
        this.addWidget(new AdvancedTooltipWidget(this, 37, 22, 50, 32).withTooltip(() -> ModUtils.getString(container1.base.workers)));
        this.addWidget(new AdvancedTooltipWidget(this, 37, 35, 50, 49).withTooltip(() -> ModUtils.getString(container1.base.doctors)));
        this.addWidget(new AdvancedTooltipWidget(this, 37, 51, 50, 64).withTooltip(() -> ModUtils.getString(container1.base.builders)));
        this.addWidget(new AdvancedTooltipWidget(this, 37, 67, 50, 80).withTooltip(() -> ModUtils.getString(container1.base.attacks)));
        this.addWidget(new AdvancedTooltipWidget(this, 37, 83, 50, 94).withTooltip(() -> ModUtils.getString(container1.base.ill)));
        this.addWidget(new AdvancedTooltipWidget(this,
                37,
                97,
                50,
    ...
    protected void drawForegroundLayer(PoseStack poseStack, int par1, int par2) {
        ...
        if (this.container.base.getGenome() != null) {
            ...
            if (maxIndexGenome == 0) {
                this.addWidget(new TooltipWidget(this, 174, 4, 20, 4 + 8 + 16 * (4)).withTooltip(() -> Localization.translate("iu.apiary_genome_descriptions")));

            } else {
                int j = 0;
                for (int i = indexGenome; i < Math.min((indexGenome) + 3, maxIndexGenome); i++, j++) {
                    int finalI = i;
                    this.addWidget(new TooltipWidget(this, 176, 8 + 16 * (j), 16, 16).withTooltip(() -> Localization.translate("iu.info.bee_genome_" + values.get(finalI).name().toLowerCase())));
            ...
            if (maxIndexAdditionProducts == 0) {
                this.addWidget(new TooltipWidget(this, 174, (int) (4 + 16 * (0 + 3.5)), 20, 4 + 8 + 16 * (4)).withTooltip(() -> Localization.translate("iu.apiary_additional_product_descriptions")));
    ...
    protected void drawGuiContainerBackgroundLayer(PoseStack poseStack, float f, int x, int y) {
        ...
        if (this.container.base.getGenome() != null) {
            ...
            for (int i = indexGenome; i < Math.min((indexGenome) + 3, maxIndexGenome); i++, j++) {
                ...
                itemRenderer.renderGuiItem(new ItemStack(IUItem.genome_bee.getStack(geneticTraits.ordinal()), 1), 176 + this.guiLeft(), 8 + 16 * (j) + this.guiTop());
