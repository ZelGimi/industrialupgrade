...
public class VerticalSliderListWidget extends AbstractWidget {
    ...
    public void render(PoseStack stack, int mouseX, int mouseY, float partialTicks) {
        ...
        blit(stack, this.x, this.y - 1, 255, 0, 4, this.height - 2);

        int sliderY = (int) (this.y + (this.height - this.sizeRender) * this.sliderPosition);
        blit(stack, this.x - 1, sliderY, 12, 13, 4, this.sizeRender);

        drawCenteredString(stack, Minecraft.getInstance().font, this.getDisplayString(), this.x + 4, this.y + this.height / 2 - 4, 0xFFFFFF);
    }
    ...
    private void updateSliderPosition(double mouseY) {
        this.sliderPosition = (float) ((mouseY - this.y) / (this.height - this.sizeRender));
        if (this.sliderPosition < 0.0F) this.sliderPosition = 0.0F;
        if (this.sliderPosition > 1.0F) this.sliderPosition = 1.0F;
