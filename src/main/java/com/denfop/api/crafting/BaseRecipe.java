...
public class BaseRecipe extends ShapedRecipe {
    ...
    public BaseRecipe(ResourceLocation id, String group, CraftingBookCategory category, ItemStack output, RecipeGrid recipeGrid, List<PartRecipe> partRecipe) {
        super(id, group, category, recipeGrid.isHasTwoX() ? 2 : 3, recipeGrid.isHasTwoY() ? 2 : 3, NonNullList.create(), output);
        this.output = output;
    ...
    public BaseRecipe(ItemStack output, RecipeGrid recipeGrid, List<PartRecipe> partRecipe) {
        super(ResourceLocation.tryParse("minecraft:minecraft"), "", CraftingBookCategory.MISC, recipeGrid.isHasTwoX() ? 2 : 3, recipeGrid.isHasTwoY() ? 2 : 3, NonNullList.create(), output);
        this.output = output;
        this.size = recipeGrid.getGrids().size();
        this.minX = recipeGrid.isHasTwoX() ? 2 : 3;
        this.minY = recipeGrid.isHasTwoY() ? 2 : 3;
