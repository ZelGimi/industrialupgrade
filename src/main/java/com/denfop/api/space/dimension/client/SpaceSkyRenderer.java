...
public final class SpaceSkyRenderer {
    ...
    private static final List<Vector3d> STARS = generateStars(900, 10842L);
    private static final ResourceLocation SUN_TEXTURE = new ResourceLocation(Constants.MOD_ID, "textures/planet/sun.png");
    ...
    private static void drawSun(
        ...
        poseStack.mulPose(Vector3f.XP.rotationDegrees(dayAngle));
    ...
    private static void drawNeighborBodies(
        ...
        for (BodyRenderEntry entry : visibleEntries) {
            ...
            poseStack.pushPose();
            poseStack.mulPose(Vector3f.YP.rotationDegrees(params.yaw()));
            poseStack.mulPose(Vector3f.XP.rotationDegrees(params.pitch()));
            poseStack.translate(0.0D, 0.0D, -params.distance());
            poseStack.mulPose(Vector3f.ZP.rotationDegrees(params.spin()));
    ...
    private static ResourceLocation resolvePlanetBelowTexture(final SpaceDimensionProfile profile) {
        ResourceLocation customSurface = new ResourceLocation(
                Constants.MOD_ID,
    ...
    private static void drawGlowQuad(
        ...
        buffer.vertex(matrix, -size, -size, -0.01F).color(red, green, blue, alpha).endVertex();
        buffer.vertex(matrix, size, -size, -0.01F).color(red, green, blue, alpha).endVertex();
        buffer.vertex(matrix, size, size, -0.01F).color(red, green, blue, alpha).endVertex();
        buffer.vertex(matrix, -size, size, -0.01F).color(red, green, blue, alpha).endVertex();
    ...
    private static void applyRingTransform(
        ...
        if (ringType == EnumRing.VERTICAL) {
            ...
            poseStack.mulPose(Vector3f.ZP.rotationDegrees(spin * 0.25F));
        } else {
            poseStack.mulPose(Vector3f.XP.rotationDegrees(tilt));
            poseStack.mulPose(Vector3f.ZP.rotationDegrees(spin * 0.25F));
        }
    ...
    private static void drawProceduralRingHalf(
        ...
        for (RingBand band : bands) {
            ...
            for (int i = 0; i < segments; i++) {
                ...
                buffer.vertex(matrix, x1o, y1o, 0.0F).color(band.red(), band.green(), band.blue(), outerAlpha).endVertex();
                buffer.vertex(matrix, x2o, y2o, 0.0F).color(band.red(), band.green(), band.blue(), outerAlpha).endVertex();
                buffer.vertex(matrix, x2i, y2i, 0.0F).color(band.red(), band.green(), band.blue(), innerAlpha).endVertex();
                buffer.vertex(matrix, x1i, y1i, 0.0F).color(band.red(), band.green(), band.blue(), innerAlpha).endVertex();
            }
        }
    ...
    private static void drawStars(
        ...
        buffer.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        for (Vector3d star : STARS) {
            float size = (float) (0.20F + (Math.abs(star.x) + Math.abs(star.y) + Math.abs(star.z)) * 0.06F);
            float x = (float) (star.x * 100.0F);
            float y = (float) (star.y * 100.0F);
            float z = (float) (star.z * 100.0F);
    ...
    private static void addStarQuad(
        ...
        buffer.vertex(matrix, x - size, y - size, z).color(1.0F, 1.0F, 1.0F, alpha).endVertex();
        buffer.vertex(matrix, x + size, y - size, z).color(1.0F, 1.0F, 1.0F, alpha).endVertex();
        buffer.vertex(matrix, x + size, y + size, z).color(1.0F, 1.0F, 1.0F, alpha).endVertex();
        buffer.vertex(matrix, x - size, y + size, z).color(1.0F, 1.0F, 1.0F, alpha).endVertex();
    }
    ...
    private static void drawTexturedQuad(
        ...
        buffer.vertex(matrix, -size, -size, 0.0F).uv(0.0F, 1.0F).endVertex();
        buffer.vertex(matrix, size, -size, 0.0F).uv(1.0F, 1.0F).endVertex();
        buffer.vertex(matrix, size, size, 0.0F).uv(1.0F, 0.0F).endVertex();
        buffer.vertex(matrix, -size, size, 0.0F).uv(0.0F, 0.0F).endVertex();
        BufferUploader.drawWithShader(buffer.end());
    ...
    private static List<Vector3d> generateStars(final int count, final long seed) {
        Random random = new Random(seed);
        List<Vector3d> stars = new ArrayList<>(count);
        ...
        for (int i = 0; i < count; i++) {
            ...
            stars.add(new Vector3d((float) (x / len), (float) (y / len), (float) (z / len)));
        }
