...
public final class SpaceRuntimeEvents {
    ...
    public static void onLivingTick(final LivingEvent.LivingTickEvent event) {
        ...
        final SpaceDimensionProfile profile = SpaceBodyProfiles.byDimensionPath(entity.getLevel().dimension().location().getPath());
        if (profile == null) {
            clearInertia(entity);
            return;
        }
    ...
    private static void applyThermalCheck(final LivingEntity entity, final SpaceDimensionProfile profile) {
        ...
        if (bodyTemperature < minSafeTemperature) {
            ...
            if (entity.tickCount % 30 == 0) {
                entity.hurt(DamageSource.FREEZE, 2.0F);
            }
            return;
        }
    ...
    private static void applyPressureCheck(final LivingEntity entity, final SpaceDimensionProfile profile) {
        ...
        entity.hurt(DamageSource.GENERIC, 1000.0F);
    }
    ...
    private static void applyGenericGravity(final LivingEntity entity, final SpaceDimensionProfile profile) {
        if (entity.isNoGravity() || entity.isOnGround() || entity.isInWater() || entity.isInLava()) {
            return;
        }
    ...
    private static Vec3 dampMotionAgainstAsteroids(final LivingEntity entity, final Vec3 motion) {
        ...
        if (Math.abs(x) > 1.0E-4D && !entity.getLevel().noCollision(entity, box.move(x, 0.0D, 0.0D))) {
            x = 0.0D;
        }
        if (Math.abs(y) > 1.0E-4D && !entity.getLevel().noCollision(entity, box.move(0.0D, y, 0.0D))) {
            y = y > 0.0D ? 0.0D : Math.max(0.0D, y * 0.10D);
        }
        if (Math.abs(z) > 1.0E-4D && !entity.getLevel().noCollision(entity, box.move(0.0D, 0.0D, z))) {
            z = 0.0D;
        }
    ...
    private static Vec3 applyInertia(final LivingEntity entity, final Vec3 motion, final JetpackControlState controlState) {
        if (entity.isOnGround() && !controlState.active) {
    ...
    public static void onLivingFall(final LivingFallEvent event) {
        final SpaceDimensionProfile profile = SpaceBodyProfiles.byDimensionPath(event.getEntity().getLevel().dimension().location().getPath());
        if (profile == null) {
            return;
        }
    ...
    private record JetpackControlState(boolean available, boolean active, boolean hoverMode) {
        ...
        private static JetpackControlState resolve(final LivingEntity entity) {
            ...
            final CompoundTag nbt = chest.getTag();
            ...
            final boolean looksLikeJetpack = simpleName.contains("jetpack")
                    || (nbt != null && (nbt.contains("jetpack") || nbt.contains("hoverMode") || nbt.contains("vertical") || nbt.contains("canFly")));

            if (!looksLikeJetpack) {
                return new JetpackControlState(false, false, false);
            }

            final boolean active = nbt != null && nbt.getBoolean("jetpack");
