...
public final class SpaceBiomesBootstrap {
    ...
    public static Map<ResourceLocation, Biome> build(final RegistryAccess registryAccess) {
        final Registry<PlacedFeature> placed = registryAccess.registryOrThrow(Registry.PLACED_FEATURE_REGISTRY);
        final Map<ResourceLocation, Biome> entries = new LinkedHashMap<>();
        ...
        for (final SpaceBodyRef body : SpaceBodyCatalog.allBodies()) {
            ...
            final BiomeGenerationSettings.Builder generation = new BiomeGenerationSettings.Builder();

            if (profile.generateAsteroidField()) {
                generation.addFeature(GenerationStep.Decoration.RAW_GENERATION, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.asteroidFieldPlacedKey(body.name())));
                ...
                register(entries, SpaceDimensionKeys.biomeKey(body.name()), new Biome.BiomeBuilder()
            ...
            if (profile.generateCraters()) {
                generation.addFeature(GenerationStep.Decoration.LOCAL_MODIFICATIONS, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.craterPlacedKey(body.name())));
            }
            generation.addFeature(GenerationStep.Decoration.LOCAL_MODIFICATIONS, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.craterAdditionPlacedKey(body.name())));

            if (profile.generateVolcanoes()) {
                generation.addFeature(GenerationStep.Decoration.LOCAL_MODIFICATIONS, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.volcanoPlacedKey(body.name())));
            }
            if (profile.generateCavities()) {
                generation.addFeature(GenerationStep.Decoration.RAW_GENERATION, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.cavityPlacedKey(body.name())));
            }
            if (profile.generateVerticalShafts()) {
                generation.addFeature(GenerationStep.Decoration.RAW_GENERATION, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.shaftPlacedKey(body.name())));
            }
            if (profile.generateLavaTubes()) {
                generation.addFeature(GenerationStep.Decoration.RAW_GENERATION, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.lavaTubePlacedKey(body.name())));
            }

            for (int i = 0; i < profile.ores().size(); i++) {
                generation.addFeature(GenerationStep.Decoration.UNDERGROUND_ORES, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.orePlacedKey(body.name(), i)));
            }
            for (int i = 0; i < profile.ores().size(); i++) {
                generation.addFeature(GenerationStep.Decoration.UNDERGROUND_ORES, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.orePlacedKey(body.name(), profile.ores().size() + i)));
            }

            for (int i = 0; i < profile.fluids().size(); i++) {
                generation.addFeature(GenerationStep.Decoration.LAKES, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.lakePlacedKey(body.name(), i)));
                if (profile.generateGeysers()) {
                    generation.addFeature(GenerationStep.Decoration.SURFACE_STRUCTURES, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.geyserPlacedKey(body.name(), i)));
                }
                if (profile.fluids().get(i).ventType() != SpaceVentType.LAVA) {
                    generation.addFeature(GenerationStep.Decoration.LAKES, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.fluidPlacedKey(body.name(), i)));
                }
            }
            ...
            if (mode == SpaceGenerationMode.ICY_SHELL) {
                generation.addFeature(
                        ...
                        placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.placedKey(bodyName, "ice_surface_cover"))
                );
            }
            ...
            if (mode == SpaceGenerationMode.CRYO_CHEMICAL) {
                generation.addFeature(
                        GenerationStep.Decoration.TOP_LAYER_MODIFICATION,
                        placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.placedKey(bodyName, "cryo_surface_cover"))
                );
            }
            ...
            if (isVolcanic(mode, profile)) {
                generation.addFeature(
                        ...
                        placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.placedKey(bodyName, "lava_hydrology"))
                );
            }
            ...
            register(entries, SpaceDimensionKeys.biomeKey(body.name()), new Biome.BiomeBuilder()
                    ...
                    .precipitation(profile.hasAtmosphere() ? Biome.Precipitation.RAIN : Biome.Precipitation.NONE)
                    .specialEffects(effects.build())
                    .mobSpawnSettings(new MobSpawnSettings.Builder().build())
                    .generationSettings(generation.build())
                    .build());
    ...
    private static void register(
            final Map<ResourceLocation, Biome> entries,
    ...
    private static void addRawFeature(
            ...
            final Registry<PlacedFeature> placed,
        ...
        generation.addFeature(GenerationStep.Decoration.RAW_GENERATION, placed.getOrCreateHolderOrThrow(SpaceDimensionKeys.placedKey(bodyName, suffix)));
    }
