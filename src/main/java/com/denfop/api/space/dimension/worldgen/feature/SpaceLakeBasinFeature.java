...
public class SpaceLakeBasinFeature extends Feature<SpaceBodyFeatureConfig> {
    ...
    private static boolean canSeal(final WorldGenLevel level, final BlockPos pos) {
        ...
        return state.isAir() || state.canBeReplaced(state.getFluidState().getType()) || !state.getFluidState().isEmpty();
    }
    ...
    private static boolean canPlaceLakeFluid(
        ...
        if (!state.isAir() && !state.canBeReplaced(state.getFluidState().getType()) && state.getFluidState().isEmpty()) {
            return false;
        }
