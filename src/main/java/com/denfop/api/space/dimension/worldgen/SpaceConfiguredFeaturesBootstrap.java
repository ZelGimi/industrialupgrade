...
public final class SpaceConfiguredFeaturesBootstrap {
    ...
    public static Map<ResourceLocation, ConfiguredFeature<?, ?>> build() {
        final Map<ResourceLocation, ConfiguredFeature<?, ?>> entries = new LinkedHashMap<>();
        for (final SpaceBodyRef body : SpaceBodyCatalog.allBodies()) {
            ...
            Map<ResourceLocation, ConfiguredFeature<?, ?>> context = entries;
    ...
    private static void register(
            final Map<ResourceLocation, ConfiguredFeature<?, ?>> entries,
            final ResourceKey<ConfiguredFeature<?, ?>> key,
            final ConfiguredFeature<?, ?> value
    ) {
    ...
    private static void registerFormation(
        ...
        registerFormation(entries, bodyName, suffix, config, SpaceWorldgenContent.ROCK_FORMATION.get());
    }
    ...
    private static void registerBodyOres(final Map<ResourceLocation, ConfiguredFeature<?, ?>> entries, final SpaceDimensionProfile body, int oreIndex) {
    ...
    private static void registerBodyFluids(
        ...
        for (final SpaceFluidPocket pocket : body.fluids()) {
            ...
            final String fluidId = Registry.FLUID.getKey(pocket.fluid()).toString();
    ...
    private static void addReplaceable(final LinkedHashSet<String> ids, final BlockState state) {
        ...
        ids.add(Registry.BLOCK.getKey(block).toString());
    }
    ...
    private static List<String> buildReplaceableBlockIds(final SpaceDimensionProfile profile) {
        ...
        if (ids.isEmpty()) {
            ids.add(Registry.BLOCK.getKey(Blocks.STONE).toString());
        }
