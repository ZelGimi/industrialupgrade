...
public class SpaceRockFormationFeature extends Feature<SpaceFormationFeatureConfig> {
    ...
    private boolean placePillars(final WorldGenLevel level, final BlockPos origin, final RandomSource random, final SpaceFormationFeatureConfig cfg) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for (int dy = 0; dy < h; dy++) {
                SpaceFeatureUtils.carveSphere(level, new BlockPos(x, y + dy, z), r, 1, r, cfg.primary(), state -> state.isAir() || state.canBeReplaced(Fluids.EMPTY));
            }
        }
        return true;
    }
    ...
    private boolean placeArch(final WorldGenLevel level, final BlockPos origin, final RandomSource random, final SpaceFormationFeatureConfig cfg) {
        ...
        for (int i = 0; i < h1; i++) {
            SpaceFeatureUtils.carveSphere(level, new BlockPos(x1, y1 + i, z1), cfg.minRadius(), 1, cfg.minRadius(), cfg.primary(), s -> s.isAir() || s.canBeReplaced(Fluids.EMPTY));
        }
        for (int i = 0; i < h2; i++) {
            SpaceFeatureUtils.carveSphere(level, new BlockPos(x2, y2 + i, z2), cfg.minRadius(), 1, cfg.minRadius(), cfg.primary(), s -> s.isAir() || s.canBeReplaced(Fluids.EMPTY));
        }

        final BlockPos bridgeStart = new BlockPos(x1, y1 + h1 - 1, z1);
        final BlockPos bridgeEnd = new BlockPos(x2, y2 + h2 - 1, z2);
        SpaceFeatureUtils.lineOfSpheres(level, bridgeStart, bridgeEnd.above(4), cfg.minRadius(), cfg.primary(), s -> s.isAir() || s.canBeReplaced(Fluids.EMPTY));
    ...
    private boolean placePlateau(final WorldGenLevel level, final BlockPos origin, final RandomSource random, final SpaceFormationFeatureConfig cfg) {
        ...
        for (int y = 0; y < thickness; y++) {
            ...
            SpaceFeatureUtils.carveSphere(level, new BlockPos(origin.getX(), centerY + y, origin.getZ()),
                    currentRadius, 1, currentRadius, cfg.primary(), s -> s.isAir() || s.canBeReplaced(Fluids.EMPTY));
        }
    ...
    private boolean placeMountainSpire(final WorldGenLevel level, final BlockPos origin, final RandomSource random, final SpaceFormationFeatureConfig cfg) {
        ...
        for (int dy = 0; dy < h; dy++) {
            ...
            SpaceFeatureUtils.carveSphere(level, new BlockPos(x, y + dy, z), radius, 1, radius, cfg.primary(), s -> s.isAir() || s.canBeReplaced(Fluids.EMPTY));
        }
        ...
        for (int x = -width; x <= width; x++) {
            ...
            for (int y = 0; y < localHeight; y++) {
                for (int z = -2; z <= 2; z++) {
                    ...
                    if (level.getBlockState(pos).isAir() || level.getBlockState(pos).canBeReplaced(Fluids.EMPTY)) {
