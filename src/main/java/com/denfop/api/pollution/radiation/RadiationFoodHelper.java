...
public final class RadiationFoodHelper {
    ...
    public static final String FOOD_RADIATION_TAG = "iu_food_radiation_dose";
    ...
    public static boolean isConsumableCandidate(ItemStack stack) {
        return stack != null
                && !stack.isEmpty()
                && (stack.isEdible() || stack.getUseAnimation() == UseAnim.DRINK);
    }
    ...
    public static FoodCategory getCategory(ItemStack stack) {
        ...
        if (stack.isEdible()) {
            if (stack.getFoodProperties(null) != null && stack.getFoodProperties(null).isMeat()) {
    ...
    public static double getSpoilThreshold(ItemStack stack) {
        switch (getCategory(stack)) {
            case MEAT:
                return MEAT_THRESHOLD;
            case VEGETABLE:
                return VEGETABLE_THRESHOLD;
            case WATER:
    ...
    public static double getFoodRadiationDose(ItemStack stack) {
        ...
        if (tag == null || !tag.contains(FOOD_RADIATION_TAG)) {
            return 0.0D;
        }
        return Math.max(0.0D, tag.getDouble(FOOD_RADIATION_TAG));
    }
    ...
    public static void setFoodRadiationDose(ItemStack stack, double dose) {
        ...
        stack.getOrCreateTag().putDouble(FOOD_RADIATION_TAG, safeDose);
    }
    ...
    public static ItemStack createSpoiledStack(ItemStack source) {
        ...
        if (source.hasCustomHoverName()) {
            spoiled.setHoverName(source.getHoverName());
        }
