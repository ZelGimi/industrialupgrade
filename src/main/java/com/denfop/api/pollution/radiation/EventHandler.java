...
public class EventHandler {
    ...
    @SubscribeEvent
    public void onFinalizeSpawn(LivingSpawnEvent.SpecialSpawn event) {
        ...
        if (event.getSpawnReason() != MobSpawnType.NATURAL && event.getSpawnReason() != MobSpawnType.REINFORCEMENT) {
            return;
        }
        ...
        DifficultyInstance difficulty = event.getLevel().getCurrentDifficultyAt(originalMob.blockPosition());

        for (int i = 0; i < extraAttempts; i++) {
            if (serverLevel.random.nextFloat() <= chance) {
                spawnBonusMob(serverLevel, originalMob, event.getSpawnReason(), difficulty, radiationOrdinal);
            }
        }
    }
    ...
    private void spawnBonusMob(
        ...
        for (int attempt = 0; attempt < 6; attempt++) {
            ...
            extraMob.finalizeSpawn(serverLevel, difficulty, spawnType, null, null);
