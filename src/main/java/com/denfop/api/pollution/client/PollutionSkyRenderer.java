...
public final class PollutionSkyRenderer {
    ...
    private static void drawHorizontalLayer(Matrix4f matrix, double centerX, double y, double centerZ, float halfSize, int rgb, float alpha) {
        ...
        builder.vertex(matrix, minX, fy, minZ).color(r, g, b, a).endVertex();
        builder.vertex(matrix, minX, fy, maxZ).color(r, g, b, a).endVertex();
        builder.vertex(matrix, maxX, fy, maxZ).color(r, g, b, a).endVertex();
        builder.vertex(matrix, maxX, fy, minZ).color(r, g, b, a).endVertex();
