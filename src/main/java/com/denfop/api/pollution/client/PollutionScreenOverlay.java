...
public enum PollutionScreenOverlay implements IGuiOverlay {
    INSTANCE;

    private static void renderBaseVeil(PoseStack guiGraphics, int width, int height, PollutionVisualData state, float pulse) {
        ...
        fill(guiGraphics, 0, 0, width, height, PollutionVisualColors.argb(alpha, state.getOverlayColor()));
    }

    private static void renderEdgeVignette(PoseStack guiGraphics, int width, int height, PollutionVisualData state, float pulse) {
        ...
        for (int i = 0; i < layers; i++) {
            ...
            fill(guiGraphics, inset, inset, width - inset, inset + 2, color);
            fill(guiGraphics, inset, height - inset - 2, width - inset, height - inset, color);
            fill(guiGraphics, inset, inset + 2, inset + 2, height - inset - 2, color);
            fill(guiGraphics, width - inset - 2, inset + 2, width - inset, height - inset - 2, color);
        }
    }

    private static void renderNoiseFilm(PoseStack guiGraphics, int width, int height, PollutionVisualData state, float partialTick) {
        ...
        for (int i = 0; i < dots; i++) {
            ...
            fill(guiGraphics, x, y, Math.min(width, x + size), Math.min(height, y + size), color);
        }
    }

    private static void renderScanlines(PoseStack guiGraphics, int width, int height, PollutionVisualData state, float pulse) {
        ...
        for (int y = offset; y < height; y += 4) {
            fill(guiGraphics, 0, y, width, y + 1, color);
        }
    }

    private static void renderAnomalyBands(PoseStack guiGraphics, int width, int height, PollutionVisualData state, float pulse) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            fill(guiGraphics, startX, 0, endX, height, color);
        }
    }
