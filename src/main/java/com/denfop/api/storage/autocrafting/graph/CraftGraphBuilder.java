...
public class CraftGraphBuilder {
    ...
    private void buildPatternCache(List<PatternStack> patterns) {
        ...
        for (PatternStack ps : patterns) {
            for (SameStack s : ps.output()) {
                if (s.isItem()) {
                    String key = Registry.ITEM.getKey(s.getStack().getItem()).toString();
                    patternCacheItem.computeIfAbsent(key, k -> new ArrayList<>()).add(ps);
                } else if (s.isFluid()) {
                    String key = Registry.FLUID.getKey(s.getFluidStack().getFluid()).toString();
    ...
    private Tuple<PatternStack, SameStack> findPatternFor(SameStack stack) {
        if (stack.isItem()) {
            List<PatternStack> list = patternCacheItem.get(Registry.ITEM.getKey(stack.getStack().getItem()).toString());
            if (list == null) return null;
            for (PatternStack ps : list) {
                for (SameStack out : ps.output()) {
                    if (out.isItem() && out.getStack().is(stack.getStack().getItem()) && ModUtils.compareNbt(out.getStack().getTag(), stack.getStack().getTag(), true)) {
        ...
        if (stack.isFluid()) {
            List<PatternStack> list = patternCacheFluid.get(Registry.FLUID.getKey(stack.getFluidStack().getFluid()).toString());
            if (list == null) return null;
            for (PatternStack ps : list) {
                for (SameStack out : ps.output()) {
                    if (out.isFluid() && out.getFluidStack().isFluidEqual(stack.getFluidStack())) {
                        return new Tuple<>(ps, out);
                    }
                }
            }
