...
public class AutoCraftSystem {
    ...
    private static CompoundTag writeReservedMap(Map<String, Map<SameStack, Integer>> src) {
        ...
        for (var cellEntry : src.entrySet()) {
            ...
            for (var e : inner.entrySet()) {
                ...
                el.put("SameStack", sameStack.writeToNBT());
    ...
    public static AutoCraftSystem readFromTag(CompoundTag tag) {
        AutoCraftOutput output = AutoCraftOutput.readFromNBT(tag.getCompound("AutoCraftOutput"));
        ...
        for (int i = 0; i < listTag.size(); i++) {
            ...
            system.autoStacks.add(AutoCraftStack.readFromNBT(stackTag));
        }
        if (tag.contains("ReservedItemSlots", 10)) {
            readReservedMap(tag.getCompound("ReservedItemSlots"), system.reservedItemSlots);
        }


        if (tag.contains("ReservedFluidSlots", 10)) {
            readReservedMap(tag.getCompound("ReservedFluidSlots"), system.reservedFluidSlots);
        }
        return system;
    }

    private static void readReservedMap(CompoundTag in, Map<String, Map<SameStack, Integer>> dst) {
        ...
        for (String cellKey : in.getAllKeys()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                SameStack sameStack = SameStack.readFromNBT(el.getCompound("SameStack"));
    ...
    public void addReserve(SameStack slot, int amount) {
        ...
        if (slot.isFluid()) {
            cellKey = Registry.FLUID.getKey(slot.getFluidStack().getFluid()).toString();
            target = getReservedFluidSlots();
        } else {
            cellKey = Registry.ITEM.getKey(slot.getStack().getItem()).toString();
    ...
    public CompoundTag writeToTag() {
        ...
        tag.put("ReservedItemSlots", writeReservedMap(this.reservedItemSlots));


        tag.put("ReservedFluidSlots", writeReservedMap(this.reservedFluidSlots));
