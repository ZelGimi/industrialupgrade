...
public class EnergyNetGlobal implements EnergyNet {

    public static EnergyNet instance;
    public static long tick = 0;
    public static EnergyTile EMPTY = new com.denfop.api.energy.EnergyTile() {
    ...
    public EnergyTile getTileEntity(final Level world, final int x, final int y, final int z) {
    ...
    public EnergyTile getTileEntity(final Level world, BlockPos pos) {
        final EnergyNetLocal local = getForWorld(world);
        if (local != EnergyNetLocal.EMPTY) {
    ...
    @Override
    public EnergyTile getTile(final Level world, final BlockPos blockPos) {
        final EnergyNetLocal local = getForWorld(world);
        if (local != EnergyNetLocal.EMPTY) {
    ...
    @Override
    public NodeStats getNodeStats(final EnergyTile te, Level level) {
    ...
    @Override
    public Map<ChunkPos, List<EnergySink>> getChunkPosListMap(final Level world) {
    ...
    @Override
    public BlockEntity getBlockPosFromEnergyTile(final EnergyTile tile, Level level) {
        final EnergyNetLocal local = getForWorld(level);
        if (local != EnergyNetLocal.EMPTY) {
    ...
    public List<Path> getEnergyPaths(final Level world, final BlockPos pos) {
        final EnergyNetLocal local = getForWorld(world);
        EnergyTile energyTile = local.getChunkCoordinatesIEnergyTileMap().get(pos);
        if (energyTile != EMPTY) {
