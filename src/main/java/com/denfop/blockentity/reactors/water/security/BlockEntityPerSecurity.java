...
public class BlockEntityPerSecurity extends BlockEntityMultiBlockElement implements ISecurity {

    public EnumTypeSecurity security;
    private Timer red_timer = new Timer(0, 2, 30);
    private Timer yellow_timer = new Timer(0, 5, 0);

    public BlockEntityPerSecurity(BlockPos pos, BlockState state) {
        super(BlockWaterReactorsEntity.water_per_security, pos, state);
    }

    @Override
    public MultiBlockEntity getTeBlock() {
        return BlockWaterReactorsEntity.water_per_security;
    }
