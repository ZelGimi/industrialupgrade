...
import com.denfop.containermenu.ContainerMenuAdvCokeOven;
...
public class BlockEntityAdvCokeOvenMain extends BlockEntityMultiBlockBase implements IMain,
        IUpdatableTileEvent,
        AudioFixer {
    ...
    public BlockEntityAdvCokeOvenMain(BlockPos pos, BlockState state) {
        super(InitMultiBlockSystem.advCokeOvenMultiBlock, BlockAdvCokeOvenEntity.adv_coke_oven_main, pos, state);
        this.full = false;
        ...
        this.pollutionSoil = this.addComponent(new SoilPollutionComponent(this, ModConfig.mechanismDouble("improved_coke_oven_controller_soil_pollution_amount", 0.2D)));
        this.pollutionAir = this.addComponent(new AirPollutionComponent(this, ModConfig.mechanismDouble("improved_coke_oven_controller_air_pollution_amount", 0.5D)));
    }

    public MultiBlockEntity getTeBlock() {
        return BlockAdvCokeOvenEntity.adv_coke_oven_main;
    }
        ...
        if (!(!this.full || !this.activate)) {
            if (!this.getWorld().isClientSide && FluidHandlerFix.getFluidHandler(player.getItemInHand(hand)) != null) {
                return ModUtils.interactWithFluidHandler(player, hand,
                        this.level.getCapability(Capabilities.FluidHandler.BLOCK, this.getBlockPos(), side)
                );
            }
        }
    ...
    @Override
    public ContainerMenuAdvCokeOven getGuiContainer(final Player entityPlayer) {
        if (!this.entityPlayerList.contains(entityPlayer)) {
            this.entityPlayerList.add(entityPlayer);
        }
        return new ContainerMenuAdvCokeOven(entityPlayer, this);
    }
    ...
    public ScreenIndustrialUpgrade<ContainerMenuBase<? extends CustomWorldContainer>> getGui(Player var1, ContainerMenuBase<? extends CustomWorldContainer> menu) {
        ...
        return new ScreenAdvCokeOven((ContainerMenuAdvCokeOven) menu);
    }
