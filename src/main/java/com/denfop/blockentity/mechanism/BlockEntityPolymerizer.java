...
import com.denfop.screen.ScreenPolymerizer;
...
public class BlockEntityPolymerizer extends BlockEntityElectricMachine implements BlockEntityUpgrade, IHasRecipe {
    ...
    public BlockEntityPolymerizer(BlockPos pos, BlockState state) {
        super(ModConfig.mechanismDouble("polymerizer_energy_storage", 100.0D), 1, 2, BlockBaseMachine3Entity.polymerizer, pos, state);
        this.progress = 0;
        this.defaultEnergyConsume = this.energyConsume = ModConfig.mechanismInt("polymerizer_energy_use", 1);
        this.defaultOperationLength = this.operationLength = 100;
        this.defaultTier = 1;
        this.defaultEnergyStorage = 100;
        this.addComponent(new SoilPollutionComponent(this, ModConfig.mechanismDouble("polymerizer_soil_pollution_amount", 0.1D)));
        this.addComponent(new AirPollutionComponent(this, ModConfig.mechanismDouble("polymerizer_air_pollution_amount", 0.1D)));
        ...
        this.fluid_handler = new FluidHandlerRecipe("polymerizer", fluids);
    ...
    public void updateEntityServer() {
        ...
        if (this.fluid_handler.output() != null && se.getEnergy() >= 5 &&
                this.fluid_handler.canOperate() && this.fluid_handler.canFillFluid() && this.energy.canUseEnergy(energyConsume)) {
            se.useEnergy(5);
            if (!this.getActive()) {
                setActive(true);
            }
    ...
    public MultiBlockEntity getTeBlock() {
        return BlockBaseMachine3Entity.polymerizer;
    }
    ...
    public ContainerMenuPolymerizer getGuiContainer(Player entityPlayer) {
        return new ContainerMenuPolymerizer(entityPlayer, this);
    }

    @Override
    @OnlyIn(Dist.CLIENT)
    public ScreenIndustrialUpgrade<ContainerMenuBase<? extends CustomWorldContainer>> getGui(Player var1, ContainerMenuBase<? extends CustomWorldContainer> menu) {
        return new ScreenPolymerizer((ContainerMenuPolymerizer) menu);
    }
