...
import com.denfop.containermenu.ContainerMenuRNACollector;
...
import com.denfop.screen.ScreenRNACollector;
...
public class BlockEntityRNACollector extends BlockEntityElectricMachine implements
    ...
    public BlockEntityRNACollector(BlockPos pos, BlockState state) {
        super(ModConfig.mechanismDouble("rna_collector_energy_storage", 200.0D), 1, 1, BlockBaseMachine3Entity.rna_collector, pos, state);
        Recipes.recipes.addInitRecipes(this);

        this.progress = 0;
        this.defaultEnergyConsume = this.energyConsume = ModConfig.mechanismInt("rna_collector_energy_use", 1);
        ...
        this.pollutionSoil = this.addComponent(new SoilPollutionComponent(this, ModConfig.mechanismDouble("rna_collector_soil_pollution_amount", 0.1D)));
        this.pollutionAir = this.addComponent(new AirPollutionComponent(this, ModConfig.mechanismDouble("rna_collector_air_pollution_amount", 0.25D)));
    }
    ...
    public ContainerMenuRNACollector getGuiContainer(final Player var1) {
        return new ContainerMenuRNACollector(var1, this);
    ...
    public ScreenIndustrialUpgrade<ContainerMenuBase<? extends CustomWorldContainer>> getGui(Player var1, ContainerMenuBase<? extends CustomWorldContainer> menu) {
        ...
        return new ScreenRNACollector((ContainerMenuRNACollector) menu);
    }
    ...
    public void init() {
        addRecipe(
                ...
                new FluidStack(FluidName.fluidethanol.getInstance().get(), 250),
                new FluidStack(FluidName.fluidbeerna.getInstance().get()
                        , 250)
        );

        addRecipe(
                new ItemStack(IUItem.plant_mixture.getItem()),
                new FluidStack(FluidName.fluidseedoil.getInstance().get(), 250),
                new FluidStack(FluidName.fluidcroprna.getInstance().get()
                        , 250)
        );
    ...
    public MultiBlockEntity getTeBlock() {
        return BlockBaseMachine3Entity.rna_collector;
    }
