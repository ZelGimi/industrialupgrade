...
import com.denfop.screen.ScreenRotorUpgrade;
...
public class BlockEntityRotorModifier extends BlockEntityInventory implements IWindUpgradeBlock, IUpdatableTileEvent {

    public final InventoryUpgrade slot;
    public final InventoryRotor rotor_slot;

    public BlockEntityRotorModifier(BlockPos pos, BlockState state) {
    ...
    public MultiBlockEntity getTeBlock() {
        return BlockBaseMachine3Entity.rotor_modifier;
    }
    ...
    @Override
    public ContainerMenuRotorUpgrade getGuiContainer(final Player entityPlayer) {
        return new ContainerMenuRotorUpgrade(this, entityPlayer);
    }
    ...
    public ScreenIndustrialUpgrade<ContainerMenuBase<? extends CustomWorldContainer>> getGui(Player var1, ContainerMenuBase<? extends CustomWorldContainer> menu) {
        ...
        return new ScreenRotorUpgrade((ContainerMenuRotorUpgrade) menu);
    }
        ...
        if (var2 == 0) {
            if (!this.rotor_slot.get(0).isEmpty()) {
                ...
                for (int i = 0; i < stacks.size(); i++) {
                    if (!stacks.get(i).isEmpty()) {
                        RotorUpgradeSystem.instance.addUpdate(this.getItemStack(), this.getWorld(), (EnumInfoRotorUpgradeModules.getFromID(((ItemRotorsUpgrade<?>) stacks.get(i).getItem()).getElement().getId())));
            ...
            if (!this.rotor_slot.get(0).isEmpty()) {
                for (int i = 0; i < this.slot.size(); i++) {
                    ...
                    NeoForge.EVENT_BUS.post(new EventRotorItemLoad(this.getWorld(), (IRotorUpgradeItem) this
                            .getItemStack().getItem(), this
                            .getItemStack()));
                }
