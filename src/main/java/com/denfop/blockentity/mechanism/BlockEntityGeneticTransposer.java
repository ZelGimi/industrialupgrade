...
public class BlockEntityGeneticTransposer extends BlockEntityElectricLiquidTankInventory implements IHasRecipe, IUpdateTick,
    ...
    public BlockEntityGeneticTransposer(BlockPos pos, BlockState state) {
        super(ModConfig.mechanismDouble("genetic_transposer_energy_storage", 300.0D), 1, ModConfig.mechanismInt("genetic_transposer_tank_capacity", 12), Fluids.fluidPredicate(FluidName.fluidbeegenetic.getInstance().get(), FluidName.fluidcropgenetic.getInstance().get()), BlockBaseMachine3Entity.genetic_transposer, pos, state);
        ...
        this.pollutionSoil = this.addComponent(new SoilPollutionComponent(this, ModConfig.mechanismDouble("genetic_transposer_soil_pollution_amount", 0.1D)));
        this.pollutionAir = this.addComponent(new AirPollutionComponent(this, ModConfig.mechanismDouble("genetic_transposer_air_pollution_amount", 0.25D)));
