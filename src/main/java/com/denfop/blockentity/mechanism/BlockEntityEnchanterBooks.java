...
public class BlockEntityEnchanterBooks extends BlockEntityElectricMachine implements IUpgradableBlock, IUpdateTick,
    ...
    public BlockEntityEnchanterBooks(BlockPos pos, BlockState state) {
        super(ModConfig.mechanismDouble("enchanting_machine_energy_storage", 400.0D), 1, 1, BlockBaseMachine3Entity.enchanter_books, pos, state);
        Recipes.recipes.addInitRecipes(this);
        this.upgradeSlot = new InventoryUpgrade(this, 4);
        this.componentUpgrade = this.addComponent(new ComponentUpgradeSlots(this, upgradeSlot));
        this.componentProgress = this.addComponent(new ComponentProgress(this, 1, (short) 400));
        this.addComponent(new SoilPollutionComponent(this, ModConfig.mechanismDouble("enchanting_machine_soil_pollution_amount", 0.1D)));
        this.addComponent(new AirPollutionComponent(this, ModConfig.mechanismDouble("enchanting_machine_air_pollution_amount", 0.1D)));
    ...
    public void addInformation(ItemStack stack, List<String> tooltip) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            tooltip.add(Localization.translate("iu.machines_work_energy")
                    + this.componentProcess.getEnergyConsume()
                    + Localization.translate("iu.machines_work_energy_type_eu"));
            tooltip.add(Localization.translate("iu.machines_work_length")
                    + this.componentProcess.getDefaultOperationLength());
        }
    ...
    @Override
    public ContainerMenuEnchanterBooks getGuiContainer(final Player player) {
        return new ContainerMenuEnchanterBooks(this, player);
    }
    ...
    public void addRecipe(ItemStack stack, ResourceKey<Enchantment> enchantmentKey, int lvl, int exp) {
        final IInputHandler input = Recipes.inputFactory;
        final CompoundTag nbt = new CompoundTag();
        nbt.putInt("exp", exp);

        ItemStack enchantedBook = createStoredEnchantedBook(enchantmentKey, lvl);

        Recipes.recipes.addRecipe(
                "enchanter_books",
                new BaseMachineRecipe(
                        new Input(
                                input.getInput(Items.BOOK),
    ...
    public void addRecipe1(ItemStack stack, ResourceKey<Enchantment> enchantmentKey, int lvl, int exp) {
        final IInputHandler input = Recipes.inputFactory;
        final CompoundTag nbt = new CompoundTag();
        nbt.putInt("exp", exp);

        ItemStack previousLevelBook = createStoredEnchantedBook(enchantmentKey, lvl - 1);
        ItemStack resultBook = createStoredEnchantedBook(enchantmentKey, lvl);

        Recipes.recipes.addRecipe(
                "enchanter_books",
                new BaseMachineRecipe(
                        new Input(
                                input.getInput(previousLevelBook),
                                input.getInput(stack)
                        ),
                        new RecipeOutput(nbt, resultBook)
                )
        );
    }

    private ItemStack createStoredEnchantedBook(ResourceKey<Enchantment> enchantmentKey, int level) {
        Holder.Reference<Enchantment> enchantment = resolveEnchantment(enchantmentKey);
        ...
        enchantedBook.set(DataComponents.STORED_ENCHANTMENTS, storedEnchantments.toImmutable());

        return enchantedBook;
    }

    private Holder.Reference<Enchantment> resolveEnchantment(ResourceKey<Enchantment> enchantmentKey) {
        return getEnchantmentLookup().getOrThrow(enchantmentKey);
    }

    private HolderLookup.RegistryLookup<Enchantment> getEnchantmentLookup() {

        return IUCore.registryAccess.lookupOrThrow(Registries.ENCHANTMENT);
    }
