...
public class BlockEntityCombMacerator extends BlockEntityMultiMachine {
    ...
    public static void addRecipe(String input, String output) {
        ...
        Iterable<Holder<Item>> holder = Registry.ITEM.getTagOrEmpty(tag);
    ...
    public void init() {
        ...
        Registry.ITEM.getTags().forEach(tag -> {
        ...
        for (int i = 0; i < BlockRaws.Type.values().length; i++) {
            addmacerator("forge:storage_blocks/" + BlockRaws.Type.values()[i].getName(), "forge:crushed/" + BlockRaws.Type.values()[i].name(), 27);
        }
        addmacerator("forge:storage_blocks/raw_iron", "forge:crushed/iron", 27);
        addmacerator("forge:storage_blocks/raw_copper", "forge:crushed/copper", 27);
        addmacerator("forge:storage_blocks/raw_gold", "forge:crushed/gold", 27);
    }

    private boolean isTagNotEmpty(String tagName) {
        return !Registry.ITEM.getTags().filter(tag -> tag.getFirst().location().getPath().equals(tagName)).toList().isEmpty();
    }
