...
import com.denfop.containermenu.ContainerMenuAdvOilRefiner;
...
import com.denfop.screen.ScreenAdvOilRefiner;
...
public class BlockEntityAdvOilRefiner extends BlockEntityElectricMachine implements IManufacturerBlock, BlockEntityUpgrade, IHasRecipe {
    ...
    @Override
    public void init() {
        Recipes.recipes.getRecipeFluid().addRecipe("adv_oil_refiner", new BaseFluidMachineRecipe(new InputFluid(
                new FluidStack(FluidName.fluidindustrialoil.getInstance().get(), 10)), Arrays.asList(
                new FluidStack(
                        FluidName.fluidblackoil.getInstance().get(),
                        4
                ), new FluidStack(FluidName.fluidmotoroil.getInstance().get(), 2))));
    ...
    public MultiBlockEntity getTeBlock() {
        return BlockAdvRefinerEntity.adv_refiner;
    }
    ...
    @Override
    public ContainerMenuAdvOilRefiner getGuiContainer(Player entityPlayer) {
        return new ContainerMenuAdvOilRefiner(entityPlayer, this);
    ...
    public ScreenIndustrialUpgrade<ContainerMenuBase<? extends CustomWorldContainer>> getGui(Player entityPlayer, ContainerMenuBase<? extends CustomWorldContainer> menu) {
        return new ScreenAdvOilRefiner((ContainerMenuAdvOilRefiner) menu);
    }
    ...
    public void updateEntityServer() {
        ...
        if (this.getLevel().getGameTime() % 20 == 0 && needUpdate) {
            needUpdate = false;
