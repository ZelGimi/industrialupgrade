...
public class BlockEntityAutoSpawner extends BlockEntityElectricMachine
        ...
        for (int i = 0; i < module_slot.size(); i++) {
            if (!this.module_slot.get(i).isEmpty()) {
                ...
                if (this.progress[i] >= this.tempprogress[i]) {
                    ...
                    entity.level = this.getWorld();

                    dropItemFromEntity(entity, DamageSource.playerAttack(player), this.loot_Tables[i], i);
