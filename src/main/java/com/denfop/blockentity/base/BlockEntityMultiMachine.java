...
import com.denfop.screen.ScreenMultiMachine;
...
public abstract class BlockEntityMultiMachine extends BlockEntityInventory implements
        AudioFixer, BlockEntityUpgrade, IUpdatableTileEvent, IHasRecipe, IMultiMachine {
    ...
    private int tick;
    private BlockEntitySolarPanel panel;

    public BlockEntityMultiMachine(EnumMultiMachine enumMultiMachine, MultiBlockEntity block, BlockPos pos, BlockState state) {
        this(1, enumMultiMachine.usagePerTick, enumMultiMachine.lenghtOperation, block, pos, state);
    }


    public BlockEntityMultiMachine(int energyconsume, int OperationsPerTick, MultiBlockEntity block, BlockPos pos, BlockState state) {
        this(1, energyconsume, OperationsPerTick, block, pos, state);
    }

    public BlockEntityMultiMachine(
            int aDefaultTier,
            int energyconsume,
            int OperationsPerTick, MultiBlockEntity block, BlockPos pos, BlockState state
    ) {
        ...
        this.componentUpgrades = this.addComponent(new ComponentUpgradeSlots(this, this.multi_process.getUpgradeSlot()) {
            ...
            public void setOverclockRates(final InventoryUpgrade invSlotUpgrade) {
                ...
                ((BlockEntityMultiMachine) this.getParent()).multi_process.setOverclockRates();
        ...
        this.input_slot = new Inventory(this, Inventory.TypeItemSlot.INPUT, 1) {
            ...
            public ItemStack set(final int index, final ItemStack content) {
                ...
                if (this.get(0).isEmpty()) {
                    ((BlockEntityMultiMachine) this.base).multi_process.inputSlots.changeAccepts(ItemStack.EMPTY);
                } else {
                    ((BlockEntityMultiMachine) this.base).multi_process.inputSlots.changeAccepts(this.get(0));
                }
                return content;
            }
    ...
    public void updateEntityServer() {
        ...
        if (solartype != null) {
            if (this.energy.getEnergy() < this.energy.getCapacity()) {
                if (panel == null) {
                    this.panel = new BlockEntitySolarPanel(solartype, (MultiBlockEntity) solartype.block.getBlock(solartype.meta).getValue(), pos, solartype.block.getBlock(solartype.meta).defaultBlockState());
    ...
    public ContainerMenuMultiMachine getGuiContainer(Player player) {
        return new ContainerMenuMultiMachine(player, this, this.sizeWorkingSlot);
    }
    ...
    public ScreenIndustrialUpgrade<ContainerMenuBase<? extends CustomWorldContainer>> getGui(Player player, ContainerMenuBase<? extends CustomWorldContainer> isAdmin) {
        ...
        return new ScreenMultiMachine((ContainerMenuMultiMachine) isAdmin);
