...
import com.denfop.api.energy.interfaces.Transformer;
...
import com.denfop.screen.ScreenTransformer;
...
public abstract class BlockEntityTransformer extends BlockEntityInventory implements
        IUpdatableTileEvent, Transformer {
    ...
    public BlockEntityTransformer(int tier, MultiBlockEntity tileBlock, BlockPos pos, BlockState state) {
    ...
    public ContainerMenuTransformer getGuiContainer(Player player) {
        return new ContainerMenuTransformer(player, this, 202);
    }

    @OnlyIn(Dist.CLIENT)
    public ScreenIndustrialUpgrade<ContainerMenuBase<?>> getGui(Player player, ContainerMenuBase<?> isAdmin) {
        return new ScreenTransformer((ContainerMenuTransformer) isAdmin);
    }
