...
import com.denfop.inventory.InventoryElectricBlock;
import com.denfop.network.IUpdatableTileEvent;
import com.denfop.network.packet.PacketStopSound;
import com.denfop.proxy.CommonProxy;
import com.denfop.screen.ScreenElectricBlock;
...
public class BlockEntityElectricBlock extends BlockEntityInventory implements
        IUpdatableTileEvent,
        Storage {
    ...
    public BlockEntityElectricBlock(double tier1, double output1, double maxStorage1, boolean chargepad, String name, MultiBlockEntity block, BlockPos pos, BlockState state) {
    ...
    public BlockEntityElectricBlock(EnumElectricBlock electricBlock, MultiBlockEntity block, BlockPos pos, BlockState state) {
    ...
    public ContainerMenuElectricBlock getGuiContainer(Player player) {
        return new ContainerMenuElectricBlock(player, this);
    }
    ...
    public ScreenIndustrialUpgrade<ContainerMenuBase<? extends CustomWorldContainer>> getGui(Player entityPlayer, ContainerMenuBase<? extends CustomWorldContainer> isAdmin) {
        return new ScreenElectricBlock((ContainerMenuElectricBlock) isAdmin);
    }
    ...
    public void onLoaded() {
        ...
        wirelessComponent.setEnergySource((EnergySource) this.energy.getDelegate());
    }
    ...
    protected void chargeitems(ItemStack itemstack, double chargefactor) {
        ...
        if (freeamount > 0.0D) {
            ...
            charge = Math.min(charge, ((EnergyItem) itemstack.getItem()).getTransferEnergy(itemstack));
    ...
    public void module_charge(Player entityPlayer) {
        ...
        if (this.movementcharge) {
            ...
            for (ItemStack armorcharged : entityPlayer.getInventory().armor) {
                if (armorcharged != null) {
                    if (armorcharged.getItem() instanceof EnergyItem && this.energy.getEnergy() > 0) {
        ...
        if (this.movementchargeitem) {
            for (ItemStack charged : entityPlayer.getInventory().items) {
                if (charged != null) {
                    if (charged.getItem() instanceof EnergyItem && this.energy.getEnergy() > 0) {
