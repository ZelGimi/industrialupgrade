...
public final class AbilityTreeSearch {
    ...
    public static List<BlockPos> collectTree(
        ...
        if (!player.getLevel().hasChunkAt(origin)) {
            return List.of();
        }

        final BlockState originState = player.getLevel().getBlockState(origin);
        ...
        while (!queue.isEmpty() && logs.size() < maxLogs) {
            ...
            if (!player.getLevel().hasChunkAt(current)) {
                continue;
            }
            ...
            final BlockState state = player.getLevel().getBlockState(current);
            if (!state.is(BlockTags.LOGS)) {
                continue;
            }

            if (state.getDestroySpeed(player.getLevel(), current) < 0.0F) {
                continue;
            }
            ...
            for (int dx = -2; dx <= 2; dx++) {
                for (int dy = -2; dy <= 2; dy++) {
                    for (int dz = -2; dz <= 2; dz++) {
                        ...
                        if (!player.getLevel().hasChunkAt(checkPos)) {
                            continue;
                        }

                        if (player.getLevel().getBlockState(checkPos).is(BlockTags.LEAVES)) {
