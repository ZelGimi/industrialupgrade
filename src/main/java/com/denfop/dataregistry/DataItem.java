...
public class DataItem<T extends Enum<T> & ISubEnum, E extends Item> {
    ...
    public DataItem(Class<T> typeClass, Class<E> itemClass) {
        this(typeClass, itemClass, IUCore.MODID);
    }

    public DataItem(Class<T> typeClass, Class<E> itemClass, String location) {
        ...
        for (T type : collections) {
            try {
                ...
                final ResourceLocation key = new ResourceLocation(location, type.getMainPath() + "/" + type.getSerializedName() + type.getOtherPart());
