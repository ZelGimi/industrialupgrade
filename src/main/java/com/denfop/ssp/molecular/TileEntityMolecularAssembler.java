...
public class TileEntityMolecularAssembler extends TileEntityInventory implements IEnergySink, IHasGui, IGuiValueProvider,
    ...
    protected static final List<AxisAlignedBB> AABBs = Arrays.asList(
            new AxisAlignedBB(0.25, 0.0, 0.25, 0.75, 1.0, 0.75),
            new AxisAlignedBB(0.05, 0.0, 0.2, 0.6, 1.0, 0.8)
    );
    ...
    public TileEntityMolecularAssembler() {
        this.inputSlot = new InvSlotProcessable<IMolecularTransformerRecipeManager.Input, ItemStack, ItemStack>(
                this,
                "input",
                1,
                IMolecularTransformerRecipeManager.RECIPES
        ) {
        ...
        this.comparator.setUpdate(() -> (this.currentRecipe == null) ? 0 : ((int) Util.lerp(
                0.0,
                15.0,
                this.energyUsed / this.currentRecipe.b.totalEU
        )));
    }
    ...
    public void readFromNBT(final NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("recipe")) {
            ...
            final MachineRecipeResult<IMolecularTransformerRecipeManager.Input, ItemStack, ItemStack> output = IMolecularTransformerRecipeManager.RECIPES
                    .apply(input, false);
            if (output != null) {
    ...
    public String getEnergyNeeded() {
        return (this.currentRecipe == null) ? "" : String.format(
                "%,d %s",
                this.currentRecipe.b.totalEU,
    ...
    public String getPercent() {
        return (this.currentRecipe == null) ? "" : String.format(
                "%,.0f%%",
                this.energyUsed * 100.0 / this.currentRecipe.b.totalEU
        );
    }
    ...
    public static final class MolecularOutput implements INetworkCustomEncoder {
        ...
        public Object decode(final IGrowingBuffer buffer) throws IOException {
            return new MolecularOutput(
                    (ItemStack) DataEncoder.decode(buffer, DataEncoder.EncodedType.ItemStack),
                    (int) DataEncoder.decode(buffer, DataEncoder.EncodedType.Integer)
            );
        }
