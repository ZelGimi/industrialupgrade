...
public class AdvancedMFSU extends TileEntityInventory implements IEnergyStorage, IHasGui {
	...
	private final int output = Configs.advancedmfsu * 4;
	private final Energy energy = (Energy) addComponent((TileEntityComponent) (new Energy(this, Configs.advancedmfsu2, EnumSet.complementOf(EnumSet.of(EnumFacing.DOWN)),
			EnumSet.of(EnumFacing.DOWN), Configs.advancedmfsu1, Configs.advancedmfsu1, false)).addManagedSlot(this.chargeSlot).addManagedSlot(this.dischargeSlot));
	...
	public ContainerBase<?> getGuiContainer(EntityPlayer player) {
		try {
			return DynamicContainer.create((IInventory) this, player, GuiParser.parse(new ResourceLocation(SuperSolarPanels.MOD_ID, "guidef/AdvancedMFSU.xml"), this.teBlock.getTeClass()));
	...
	public GuiScreen getGui(EntityPlayer player, boolean b) {
		try {
			return DynamicGui.create((IInventory) this, player, GuiParser.parse(new ResourceLocation(SuperSolarPanels.MOD_ID, "guidef/AdvancedMFSU.xml"), this.teBlock.getTeClass()));
		} catch (Exception exception) {
			return null;
		}
	}
