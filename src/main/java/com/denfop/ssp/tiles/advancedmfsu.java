...
public class advancedmfsu extends TileEntityInventory implements IEnergyStorage, IHasGui {
  private int output = Configs.advancedmfsu*4;
  ...
  private Energy energy = (Energy)addComponent((TileEntityComponent)(new Energy((TileEntityBlock)this, Configs.advancedmfsu2, EnumSet.complementOf((EnumSet)EnumSet.of(EnumFacing.DOWN)), 
        EnumSet.of(EnumFacing.DOWN), Configs.advancedmfsu1, Configs.advancedmfsu1, false)).addManagedSlot((InvSlot)this.chargeSlot).addManagedSlot((InvSlot)this.dischargeSlot));
  ...
  public ContainerBase<?> getGuiContainer(EntityPlayer player) {
    try {
      return (ContainerBase<?>)DynamicContainer.create((IInventory)this, player, GuiParser.parse(new ResourceLocation("super_solar_panels", "guidef/advancedmfsu.xml"), this.teBlock.getTeClass()));
  ...
  public GuiScreen getGui(EntityPlayer player, boolean b) {
    try {
      return (GuiScreen)DynamicGui.create((IInventory)this, player, GuiParser.parse(new ResourceLocation("super_solar_panels", "guidef/advancedmfsu.xml"), this.teBlock.getTeClass()));
    } catch (Exception exception) {
      return null;
    } 
  }
