...
public abstract class TileEntityMoonPanel1 extends TileEntityInventory implements IEnergySource, IHasGui, IGuiValueProvider {
...
private int nightPower1;
  ...
  public TileEntityMoonPanel1(SolarConfig config) {
    this( config.nightPower, config.storage, config.tier);
  }
  
  public TileEntityMoonPanel1( int nightPower, int storage, int tier) {
  ...
  public void checkTheSky() {
    ...
    if (this.hasSky && this.world.canBlockSeeSky(this.pos.up())) {
      if (this.world.isDaytime() && (!this.canRain || (!this.world.isRaining() && !this.world.isThundering()))) {
        ...
        this.active = GenerationState.Nighty1;
      } 
    } else {
      this.active = GenerationState.NONE;
    }  
  ...
  public ContainerBase<? extends TileEntityMoonPanel1> getGuiContainer(EntityPlayer player) {
    return (ContainerBase<? extends TileEntityMoonPanel1>)DynamicContainer.create(this, player, GuiParser.parse(this.teBlock));
  }
  ...
  public boolean getGuiState(String name) {
    ...
    if ("moonlight".equals(name))
        return (this.active == GenerationState.Nighty1); 
  ...
  public String getOutput() {
    switch (this.active) {
    case Nighty1:
        return String.format("%s %d %s", new Object[] { Localization.translate("super_solar_panels.gui.generating"), Integer.valueOf((int) (this.nightPower*1.5)), Localization.translate("ic2.generic.text.EUt") });
