...
public class UltimateMFSU extends TileEntityInventory implements IEnergyStorage, IHasGui {
	...
	private final int output = Configs.ultimatemfsu * 4;
	private final Energy energy = (Energy) addComponent((TileEntityComponent) (new Energy(this, Configs.ultimatemfsu2, EnumSet.complementOf(EnumSet.of(EnumFacing.DOWN)),
			EnumSet.of(EnumFacing.DOWN), Configs.ultimatemfsu1, Configs.ultimatemfsu1, false)).addManagedSlot(this.chargeSlot).addManagedSlot(this.dischargeSlot));
	...
	public ContainerBase<?> getGuiContainer(EntityPlayer player) {
		try {
			return DynamicContainer.create((IInventory) this, player, GuiParser.parse(new ResourceLocation(Constants.MOD_ID, "guidef/UltimateMFSU.xml"), this.teBlock.getTeClass()));
	...
	public GuiScreen getGui(EntityPlayer player, boolean b) {
		try {
			return DynamicGui.create((IInventory) this, player, GuiParser.parse(new ResourceLocation(Constants.MOD_ID, "guidef/UltimateMFSU.xml"), this.teBlock.getTeClass()));
		} catch (Exception exception) {
			return null;
		}
	}
