...
public abstract class BaseMFSU extends TileEntityInventory implements IEnergyStorage, IHasGui, IWrenchable {
    ...
    public BaseMFSU(int output, int tier, double capacity) {
        ...
        this.energy = addComponent((new Energy(this, capacity, EnumSet.complementOf(EnumSet.of(EnumFacing.DOWN)),
                EnumSet.of(EnumFacing.DOWN), tier
        )).addManagedSlot(dischargeSlot));
    }
    ...
    public ContainerBase<?> getGuiContainer(EntityPlayer player) {
        try {
            return DynamicContainer.create(
                    this,
                    player,
                    GuiParser.parse(SuperSolarPanels.getIdentifier("guidef/advmfsu.xml"), this.teBlock.getTeClass())
            );
    ...
    public GuiScreen getGui(EntityPlayer player, boolean b) {
        try {
            return BackgroundlessDynamicGUI.create(
                    (IInventory) this,
                    player,
                    GuiParser.parse(SuperSolarPanels.getIdentifier("guidef/advmfsu.xml"), this.teBlock.getTeClass())
            );
    ...
    public String getStorageText() {
        return String.format(
                "%s %s / %s%s",
                Localization.translate(Constants.MOD_ID + ".gui.storage"),
                Util.toSiString(this.energy.getEnergy(), 3),
                Util.toSiString(this.energy.getCapacity(), 3),
                Localization.translate("ic2.generic.text.EUt")
        );
    }
    ...
    public String getOutputText() {
        return String.format(
                "%s %s%s",
                Localization.translate(Constants.MOD_ID + ".gui.maxOutput"),
                Util.toSiString(this.output, 3),
                Localization.translate("ic2.generic.text.EUt")
        );
    }
