...
public class GuiVeinSensor<T extends ContainerVeinSensor> extends GuiIU<ContainerVeinSensor> implements GuiPageButtonList.GuiResponder,
                ...
                if (!integerList.isEmpty()) {
                    ...
                    if (!find) {
                        colors[entry.getKey().getX() - container.base.getVector().getX()][entry
                                ...
                                .getZ() - container.base
                                .getVector()
                                .getZ()] = MaterialColor.STONE.col | -16777216;
                    }
    ...
    public void render(@NotNull PoseStack graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (tick == 400 || update) {
            ...
            ChunkPos pos = new ChunkPos(new BlockPos(minecraft.player.getX(), minecraft.player.getY(), minecraft.player.getZ()));
            int i = 0;
            for (int x = -4; x < 5; x++) {
                for (int z = -4; z < 5; z++) {
                    ChunkPos chunkPos = new ChunkPos(pos.x + x, pos.z + z);
                    LevelChunk chunk = minecraft.player.getLevel().getChunk(chunkPos.x, chunkPos.z);
    ...
    protected void drawGuiContainerBackgroundLayer(PoseStack poseStack, final float partialTicks, final int mouseX, final int mouseY) {
        ...
        Matrix4f martix = poseStack.last().pose();
