...
public class GuiResearchTableSpace<T extends ContainerResearchTableSpace> extends GuiIU<ContainerResearchTableSpace> implements GuiPageButtonList.GuiResponder,
    ...
    private void renderRings(PoseStack poseStack, IPlanet planet) {
        ...
        poseStack.translate(planetX, planetY, 0);
        if (!isSaturn) {
            poseStack.mulPose(Vector3f.ZP.rotationDegrees(270));
        }
        poseStack.scale((float) ((0.25 / 128D) * size), (float) ((0.25 / 128D) * size / 32), 1);
    ...
    public void renderStars(PoseStack poseStack, int x, int y, int width, int height, int starCount) {
        ...
        for (float[] star : cachedStars) {
            ...
            buffer.vertex(matrix, star[0]+ leftPos - 120, star[1]+ topPos, 0).endVertex();
            buffer.vertex(matrix, star[0]*1.001f+ leftPos - 120, star[1]*1.001f+ topPos, 0).endVertex();
            tessellator.end();
        }
    ...
    protected void drawGuiContainerBackgroundLayer(PoseStack poseStack, final float partialTicks, final int mouseX, final int mouseY) {
        ...
        if (mode == 0) {
            ...
            for (int i = 0; i < systems.size(); i++) {
                ...
                renderPlanet(poseStack, 16f, star.getLocation(), (float) this.guiLeft + 25, (float) this.guiTop + 20 + i * 32, 0,
            ...
            if (container.base.colony != null) {
                ...
                renderIconWithProgressBar(poseStack, this.guiLeft + 129, this.guiTop + 34, 20, 124,
                        0f
                );
                ...
                this.renderStars1(poseStack, this.guiLeft + 130, this.guiTop + 44 + 9, 17, 85, 400);

                renderPlanet(poseStack, 15f, SpaceInit.earth.getLocation(), (float) this.guiLeft + 139,
                        (float) this.guiTop + 44
                        , 0,
                        (float) 0,
                        0
                );
                renderPlanet(poseStack, 15f, container.base.colony.getBody().getLocation(), (float) this.guiLeft + 139,
            ...
            if (this.container.base.fakeBody != null && container.base.fakeBody.matched(satellite)) {
                renderFakeBody(poseStack, this.container.base.fakeBody);
            }
            renderDistance(poseStack, satellite);
        } else if (mode == 4) {
            drawInfoBody(poseStack, asteroid);
            renderPlanet(poseStack, 4f, asteroid.getLocation(), (float) this.guiLeft + 20,
                    (float) this.guiTop + 28, 0,
                    (float) asteroid.getRotation(this.container.base.getWorld().getGameTime() / 4D),
                    0
            );
            renderPlanet(poseStack, 4f, asteroid.getLocation(), (float) this.guiLeft + 35,
                    (float) this.guiTop + 35, 0,
                    (float) asteroid.getRotation(this.container.base.getWorld().getGameTime() / 4D),
                    0
            );
            renderPlanet(poseStack, 4f, asteroid.getLocation(), (float) this.guiLeft + 50,
                    (float) this.guiTop + 25, 0,
                    (float) asteroid.getRotation(this.container.base.getWorld().getGameTime() / 4D),
                    0
            );
            renderPlanet(poseStack, 4f, asteroid.getLocation(), (float) this.guiLeft + 31,
                    (float) this.guiTop + 45, 0,
                    (float) asteroid.getRotation(this.container.base.getWorld().getGameTime() / 4D),
                    0
            );
            renderPlanet(poseStack, 4f, asteroid.getLocation(), (float) this.guiLeft + 43,
            ...
            renderDistance(poseStack, asteroid);
        } else if (mode == 2) {
            ...
            for (int i = value2, j = 0; i < Math.min(planet.getSatelliteList().size(), value2 + 2); i++, j++) {
                ...
                renderPlanet(poseStack, 16f, planet1.getLocation(), (float) this.guiLeft + 205,
                ...
                renderIconWithProgressBar(poseStack,
                        this.guiLeft + 195,
            ...
            renderDistance(poseStack, planet);
        } else if (mode == 1) {
            ...
            if (focusedPlanet != null) {
                ...
                poseStack.pushPose();
                poseStack.translate(centerX - planetX + offsetX, centerY - planetY + offsetY, 0);
                ...
                drawLine(poseStack, buffer, (-squareSize / 2), -squareSize / 2, -squareSize / 4D, -squareSize / 2);
                drawLine(poseStack, buffer, (squareSize / 2), -squareSize / 2, squareSize / 4D, -squareSize / 2);

                drawLine(poseStack, buffer, squareSize / 2, -squareSize / 2, squareSize / 2, -squareSize / 4);
                drawLine(poseStack, buffer, squareSize / 2, squareSize / 2, squareSize / 2, squareSize / 4);
                ...
                poseStack.translate(this.guiLeft + 7 + 162 / 2D + offsetX - planetX,
                        this.guiTop + 80 + 82 / 2D + offsetY - planetY, 0
                );
            }

            poseStack.scale(scale * 16, scale * 16, 1.0F);

            bindTexture(star.getLocation());
            final double size = star.getSize();
            poseStack.pushPose();
            poseStack.translate(0, 0, 0);
            poseStack.scale((float) ((0.25 / 128D) * size), (float) ((0.25 / 128D) * size), 1);
            drawTexturedModalRect(poseStack, -128, -128, 0, 0, 256, 256);
            poseStack.popPose();
            poseStack.pushPose();
            ...
            for (IPlanet planet : star.getPlanetList()) {
                if (planet != SpaceInit.ceres) {
                    renderOrbit(poseStack, planet.getDistance());
                }
            }
            poseStack.popPose();
            for (IPlanet planet : star.getPlanetList()) {
            ...
            renderPlanet(poseStack, 32f, star.getLocation(), (float) this.guiLeft + 36, (float) this.guiTop + 45, 0,
            ...
            for (int i = value1, j = 0; i < Math.min(
                ...
                if (i < star.getPlanetList().size()) {
                    ...
                    renderIconWithProgressBar(poseStack, this.guiLeft + 195, this.guiTop + 32 + (j % 7) * 32, progressWidth, progressHeight,
                            progress
                    );
                } else {
                    IAsteroid asteroid1 = star.getAsteroidList().get(i - star.getPlanetList().size());
                    renderPlanet(poseStack, 4f, asteroid1.getLocation(), (float) this.guiLeft + 205,
                            (float) this.guiTop + 20 + (j % 7) * 32
                            , 0,
    ...
    private void drawInfoBody(PoseStack poseStack, IBody planet) {
        ...
        if (!(planet instanceof IAsteroid)) {
            renderPlanet(poseStack, 32f, planet.getLocation(), (float) this.guiLeft + 36,
                    (float) this.guiTop + 45, 0,
        ...
        renderIconWithProgressBar(poseStack, this.guiLeft + 10, this.guiTop + 20 + 55, progressWidth, progressHeight,
                progress
        );
        ...
        if (planet != SpaceInit.earth) {
            renderIconWithProgressBar(poseStack, this.guiLeft + 8, this.guiTop + 135, 165, 29,
                    0f
            );
            RenderSystem.setShaderColor(1.0F, 0.2F, 1.0F, 1.0F);
            bindTexture(new ResourceLocation(Constants.MOD_ID, "textures/gui/common2.png"));
        ...
        for (int i = 0; i < planet.getResources().size(); i++) {
            poseStack.pushPose();
            poseStack.translate(guiLeft + (int) ((15 + (i % 11) * 13)), guiTop + (int) ((27 + 66 + 13 * (i / 11))), (100.0F + itemRenderer.blitOffset));
            ...
            if (baseResource.getItemStack() != null || baseResource.getFluidStack() != null) {
                if (baseResource.getPercentPanel() <= data.getPercent() && baseResource.getItemStack() != null) {
                    ...
                    poseStack.translate(0.0D, 0.0D, (double)(itemRenderer.blitOffset + 200.0F));
                    font.drawShadow(poseStack, s, 10f, 6f / 0.75f,
                            16777215
                    );
    ...
    private void renderSatellite(PoseStack poseStack, IBody planet, IBody satellite) {
        ...
        poseStack.pushPose();
        poseStack.translate(planetX, planetY, 0); // Без лишнего смещения на size1
        poseStack.scale((float) ((0.25 / 128D) * size), (float) ((0.25 / 128D) * size), 1);
    ...
    private void renderTooltipResource(PoseStack poseStack, IBody asteroid, Data data, int par1, int par2) {
    ...
    private void renderTooltipFakeBody(PoseStack poseStack, IFakeBody fakePlanet, int par1, int par2) {
