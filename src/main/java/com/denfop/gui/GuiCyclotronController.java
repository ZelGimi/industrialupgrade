...
public class GuiCyclotronController<T extends ContainerCyclotronController> extends GuiIU<ContainerCyclotronController> {
    ...
    protected void drawGuiContainerBackgroundLayer(GuiGraphics poseStack, float f, int x, int y) {
       ...
       RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
        ...
        (new FluidItem(this, 33, 28, container.base.coolant.getCoolantTank().getFluid()) {
            ...
            @Override
            public void drawForeground(GuiGraphics poseStack1, final int mouseX, final int mouseY) {
                if (mouseX >= this.x - 4 && mouseX <= this.x + 15 && mouseY >= this.y - 4 && mouseY <= this.y + 51) {
                    List<String> lines = this.getToolTip();
                    if (this.getTooltipProvider() != null) {
                        String tooltip = this.getTooltipProvider().get();
    ...
    protected void drawForegroundLayer(GuiGraphics poseStack, final int par1, final int par2) {
        ...
        new ItemStackImage(this, 103, 31, () -> this.container.base.electrostaticDeflector.getOutputSlot().get(0)).drawForeground(
                poseStack, par1,
                par2
        );
