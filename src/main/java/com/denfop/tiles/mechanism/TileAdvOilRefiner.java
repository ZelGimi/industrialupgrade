...
public class TileAdvOilRefiner extends TileElectricMachine implements IManufacturerBlock, IUpgradableBlock, IHasRecipe {
    ...
    public void init() {
        Recipes.recipes.getRecipeFluid().addRecipe("adv_oil_refiner", new BaseFluidMachineRecipe(new InputFluid(
                new FluidStack(FluidName.fluidindustrialoil.getInstance(), 10)), Arrays.asList(
                new FluidStack(
                        FluidName.fluidblackoil.getInstance(),
                        4
                ), new FluidStack(FluidName.fluidmotoroil.getInstance(), 2))));
