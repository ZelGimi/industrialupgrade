...
public class TileEntityPlantFertilizer extends TileEntityInventory  implements IUpgradableBlock {
    ...
    @Override
    public ContainerPlantFertilizer getGuiContainer(final EntityPlayer var1) {
    ...
    public GuiScreen getGui(final EntityPlayer var1, final boolean var2) {
        return new GuiPlantFertilizer(getGuiContainer(var1));
    }
            ...
            for (List<TileEntityCrop> crops : list) {
                for (TileEntityCrop crop : crops) {
                    if (this.energy.getEnergy() > 5 && !this.slot.get().isEmpty()) {
                        if (crop.getPestUse() < 40 && this.contains(crop.getPos()) && crop.getCrop() != null && crop.getCrop().getId() != 3) {
