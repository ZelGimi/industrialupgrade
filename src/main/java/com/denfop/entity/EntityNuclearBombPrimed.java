...
public class EntityNuclearBombPrimed extends Entity {
    ...
    public void tick() {
        ...
        if (fuse <= 0) {
            ...
            this.getLevel().addParticle(net.minecraft.core.particles.ParticleTypes.SMOKE,
                    this.getX(), this.getY() + 0.5, this.getZ(), 0, 0, 0);
        }
    }

    private void explode() {
        getLevel().explode(this, this.getX(), this.getY(0.0625D), this.getZ(), 20.0F, Explosion.BlockInteraction.BREAK);
        BlockPos explosionPos = new BlockPos(this.getX(), this.getY(), this.getZ());

        if (!this.getLevel().isClientSide) {
            if (this.getLevel().dimension() == Level.OVERWORLD) {
                int radiationValue = 5000;
                new PacketUpdateRadiationValue(this.getLevel().getChunk(explosionPos).getPos(), radiationValue);
    ...
    private void spreadRadiation(BlockPos explosionPos, int initialRadiation) {
        ...
        for (int x = -radius; x <= radius; x++) {
            for (int z = -radius; z <= radius; z++) {
                ...
                new PacketUpdateRadiationValue(this.getLevel().getChunk(chunkCenter).getPos(), radiationLevel);
            }
        }
    }

    private void affectNearbyPlayers(BlockPos explosionPos, double radius) {
        List<Player> players = this.getLevel().getEntitiesOfClass(
                Player.class,
