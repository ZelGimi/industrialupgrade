...
public class SmallBee extends net.minecraft.world.entity.animal.Bee {
    ...
    protected void defineSynchedData() {
        ...
        this.entityData.define(DATA_BEE_ID, -1);
        this.entityData.define(DATA_BEE_NAME, "");
    }
    ...
    private boolean isHivePosUsable(BlockPos pos) {
        ...
        return this.getLevel().getBlockEntity(pos) instanceof BlockEntityApiary;
    }
    ...
    private boolean isLoadedCropValid(BlockPos pos) {
        ...
        return this.getLevel().getBlockEntity(pos) instanceof TileEntityCrop crop && crop.getCrop() != null;
    }
    ...
    public boolean ensureCropTarget() {
        ...
        if (this.currentCropTarget != null && this.getLevel().hasChunkAt(this.currentCropTarget) && !this.isLoadedCropValid(this.currentCropTarget)) {
    ...
    private void removeInvalidLoadedCropTargets() {
        this.cropTargets.removeIf(pos -> this.getLevel().hasChunkAt(pos) && !this.isLoadedCropValid(pos));
    }
    ...
    public void finishAtCrop() {
        ...
        if (!this.getLevel().isClientSide()) {
            ...
            this.getLevel().levelEvent(1505, this.currentCropTarget, 0);
        }
    ...
    public void moveTowardsCurrentCrop() {
        ...
        if (!this.getLevel().hasChunkAt(this.currentCropTarget)) {
            return;
        }
    ...
    public void moveTowardsHive() {
        ...
        if (!this.getLevel().hasChunkAt(this.hivePos)) {
            return;
        }
    ...
    public boolean pathfindRandomlyTowards(Vec3 targetCenter) {
        ...
        BlockPos targetPos = containing(targetCenter);

        if (!this.getLevel().hasChunkAt(targetPos)) {
            return false;
        }
    ...
    private void bootstrapFromWorld() {
        ...
        if (this.bee == null && this.hivePos != null && this.getLevel().getBlockEntity(this.hivePos) instanceof BlockEntityApiary apiary) {
    ...
    private BlockPos findNearbyApiary() {
        BlockPos origin = containing(this.position());

        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                for (int dz = -1; dz <= 1; dz++) {
                    BlockPos checkPos = origin.offset(dx, dy, dz);
                    if (this.getLevel().getBlockEntity(checkPos) instanceof BlockEntityApiary) {
    ...
    public void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains(TAG_CUSTOM_HIVE_POS, Tag.TAG_COMPOUND)) {
            this.hivePos = NbtUtils.readBlockPos(tag.getCompound(TAG_CUSTOM_HIVE_POS)).immutable();
        } else if (tag.contains(TAG_LEGACY_HIVE_POS, Tag.TAG_COMPOUND)) {
            this.hivePos = NbtUtils.readBlockPos(tag.getCompound(TAG_LEGACY_HIVE_POS)).immutable();
        } else {
            this.hivePos = null;
        }

        if (tag.contains(TAG_CURRENT_CROP_TARGET, Tag.TAG_COMPOUND)) {
            this.currentCropTarget = NbtUtils.readBlockPos(tag.getCompound(TAG_CURRENT_CROP_TARGET)).immutable();
        } else {
            this.currentCropTarget = null;
        }
        ...
        if (tag.contains(TAG_CROP_TARGETS, Tag.TAG_LIST)) {
            ...
            for (int i = 0; i < cropList.size(); i++) {
                this.cropTargets.add(NbtUtils.readBlockPos(cropList.getCompound(i)).immutable());
            }
        }
        ...
        if (!this.getLevel().isClientSide() && beeId >= 0) {
            Bee resolved = BeeNetwork.instance.getBee(beeId);
            this.bee = resolved;
        } else {
            this.bee = null;
