...
public class AdvAlloySmelterCategory extends GuiIU implements IRecipeCategory<AdvAlloySmelterHandler> {
    ...
    @Override
    public void draw(AdvAlloySmelterHandler recipe, IRecipeSlotsView recipeSlotsView, PoseStack poseStack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
        ...
        Minecraft.getInstance().font.draw(poseStack, "" + temp + "°C", 82, 55, 4210752);
