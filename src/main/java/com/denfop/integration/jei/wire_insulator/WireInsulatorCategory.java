...
public class WireInsulatorCategory extends ScreenMain implements IRecipeCategory<WireInsulatorHandler> {
    ...
    @Override
    public RecipeType<WireInsulatorHandler> getRecipeType() {
    ...
    public String getTitles() {
        return Localization.translate(JEICompat.getBlockStack(BlockBaseMachine3Entity.electric_wire_insulator).getDescriptionId());
    }
    ...
    @Override
    public void draw(WireInsulatorHandler recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, WireInsulatorHandler recipe, IFocusGroup focuses) {
