...
public class SynthesisCategory extends ScreenMain implements IRecipeCategory<SynthesisHandler> {
    ...
    @Override
    public void draw(SynthesisHandler recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        progress++;
        energy++;
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, SynthesisHandler recipe, IFocusGroup focuses) {
        builder.addSlot(RecipeIngredientRole.INPUT, 20, 31).addItemStacks(JeiIngredientHelper.getInputVariants(recipe, 0, recipe.getInput()));
        builder.addSlot(RecipeIngredientRole.INPUT, 56, 31).addItemStacks(JeiIngredientHelper.getInputVariants(recipe, 1, recipe.getInput1()));
