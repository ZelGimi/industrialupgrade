...
public class RollingCategory extends ScreenMain implements IRecipeCategory<RollingHandler> {
    ...
    public RollingCategory(
        ...
        super(new ContainerMenuMultiMachine(Minecraft.getInstance().player,
                ((BlockEntityRolling) BlockMoreMachine2Entity.rolling.getDummyTe()), 1, true
        ));
    ...
    @Override
    public void draw(RollingHandler recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
