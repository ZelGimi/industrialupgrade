...
public class CombMacCategory extends ScreenMain implements IRecipeCategory<CombMacHandler> {
    ...
    @Override
    public void draw(CombMacHandler recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
