...
public class DistillerCategory extends ScreenMain implements IRecipeCategory<DistillerHandler> {
    ...
    public String getTitles() {
        return Localization.translate(JEICompat.getBlockStack(BlockBaseMachine3Entity.solardestiller).getDescriptionId());
    }
    ...
    @Override
    public void draw(DistillerHandler recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
    ...
    @Override
    public RecipeType<DistillerHandler> getRecipeType() {
