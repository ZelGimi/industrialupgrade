...
public class SolidMixerCategory extends ScreenMain implements IRecipeCategory<SolidMixerHandler> {
    ...
    @Override
    public void draw(SolidMixerHandler recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, SolidMixerHandler recipe, IFocusGroup focuses) {
        ...
        int i = 0;
        for (; i < inputs.size(); i++) {
            builder.addSlot(RecipeIngredientRole.INPUT, slots1.get(i).getJeiX() - 20, slots1.get(i).getJeiY() - 10).addItemStacks(JeiIngredientHelper.getInputVariants(recipe, i, inputs.get(i)));
