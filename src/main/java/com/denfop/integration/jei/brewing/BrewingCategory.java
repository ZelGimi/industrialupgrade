...
public class BrewingCategory extends ScreenMain implements IRecipeCategory<BrewingHandler> {
    ...
    public String getTitles() {
        return Localization.translate(JEICompat.getBlockStack(BlockBaseMachine3Entity.electric_brewing).getDescriptionId());
    }


    @Override
    public RecipeType<BrewingHandler> getRecipeType() {
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, BrewingHandler recipes, IFocusGroup focuses) {
    ...
    @Override
    public void draw(BrewingHandler recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
