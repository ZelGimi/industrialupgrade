...
public class EnvironmentalTransformationCategory extends ScreenMain implements IRecipeCategory<EnvironmentalTransformationHandler>{
    ...
    private void drawPanel(PoseStack guiGraphics) {
        fill(guiGraphics,0, 0, WIDTH, HEIGHT, PANEL_BORDER_DARK);
        fill(guiGraphics,1, 1, WIDTH - 1, HEIGHT - 1, PANEL_BORDER_LIGHT);
        fill(guiGraphics,3, 3, WIDTH - 3, HEIGHT - 3, PANEL_INNER_BORDER);
        fill(guiGraphics,5, 5, WIDTH - 5, HEIGHT - 5, PANEL_COLOR);
    }

    private void drawArrow(PoseStack guiGraphics, int x, int y, int width) {
        Font font = Minecraft.getInstance().font;
        int centerY = y + 5;
        hLine(guiGraphics,x, x + width - 8, centerY, 0xFF606060);
        drawString(guiGraphics,font, ">", x + width - 7, y, MUTED_TEXT_COLOR);
    }
    ...
    private void drawWrappedText(
        ...
        for (int i = 0; i < linesToDraw; i++) {
            drawString(guiGraphics,font, lines.get(i), x, y + i * LINE_HEIGHT, color);
        }
    }
    ...
    private void drawWrappedCenteredText(
        ...
        for (int i = 0; i < linesToDraw; i++) {
            ...
            drawString(guiGraphics,font, line, centerX - font.width(line) / 2, y + i * LINE_HEIGHT, color);
        }
    }
