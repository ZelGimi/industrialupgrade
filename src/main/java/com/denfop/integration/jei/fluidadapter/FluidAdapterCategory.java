...
public class FluidAdapterCategory extends ScreenMain implements IRecipeCategory<FluidAdapterHandler> {
    ...
    @Override
    public RecipeType<FluidAdapterHandler> getRecipeType() {
    ...
    @Override
    public void draw(FluidAdapterHandler recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        progress++;
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, FluidAdapterHandler recipe, IFocusGroup focuses) {
        ...
        builder.addSlot(RecipeIngredientRole.INPUT, 61 - 21, 44).addItemStacks(JeiIngredientHelper.getInputVariants(recipe, 0, recipe.getInput()));
        builder.addSlot(RecipeIngredientRole.INPUT, 116 - 46, 44).addItemStacks(JeiIngredientHelper.getInputVariants(recipe, 1, recipe.getOutput()));
