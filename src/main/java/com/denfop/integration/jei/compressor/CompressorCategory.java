...
public class CompressorCategory extends ScreenMain implements IRecipeCategory<CompressorHandler> {
    ...
    public String getTitles() {
        return Localization.translate(ItemStackHelper.fromData(IUItem.simplemachine, 1, 1).getDescriptionId());
    }
    ...
    @Override
    public void draw(CompressorHandler recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
    ...
    @Override
    public RecipeType<CompressorHandler> getRecipeType() {
        return jeiInform.recipeType;
    }

    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, CompressorHandler recipes, IFocusGroup focuses) {
