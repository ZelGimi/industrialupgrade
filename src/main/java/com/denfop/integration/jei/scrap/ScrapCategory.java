...
import com.denfop.blockentity.mechanism.multimechanism.simple.BlockEntityAssamplerScrap;
...
public class ScrapCategory extends ScreenMain implements IRecipeCategory<ScrapHandler> {
    ...
    public ScrapCategory(
        ...
        super(new ContainerMenuMultiMachine(Minecraft.getInstance().player,
                ((BlockEntityAssamplerScrap) BlockMoreMachine3Entity.assamplerscrap.getDummyTe()), 1, true
        ));
    ...
    @Override
    public void draw(ScrapHandler recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
