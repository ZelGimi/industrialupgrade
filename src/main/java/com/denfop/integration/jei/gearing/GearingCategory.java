...
public class GearingCategory extends ScreenMain implements IRecipeCategory<GearingHandler> {
    ...
    public String getTitles() {
        return Localization.translate(ItemStackHelper.fromData(IUItem.machines_base3, 1, 16).getDescriptionId());
    }
    ...
    @Override
    public RecipeType<GearingHandler> getRecipeType() {
        return jeiInform.recipeType;
    }

    @Override
    public void draw(GearingHandler recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder layout, GearingHandler recipe, IFocusGroup focuses) {
        ...
        int i = 0;
        for (; i < inputs.size(); i++) {
            layout.addSlot(RecipeIngredientRole.INPUT, slots1.get(i).getJeiX(), slots1.get(i).getJeiY()).addItemStacks(JeiIngredientHelper.getInputVariants(recipe, i, inputs.get(i)));
