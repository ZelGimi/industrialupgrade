...
public class TripleSolidMixerCategory extends ScreenMain implements IRecipeCategory<TripleSolidMixerHandler> {
    ...
    @Override
    public void draw(TripleSolidMixerHandler recipe, IRecipeSlotsView recipeSlotsView, PoseStack stack, double mouseX, double mouseY) {
        if (this.energy < 100) {
            energy++;
        }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, TripleSolidMixerHandler recipe, IFocusGroup focuses) {
        ...
        int i = 0;
        for (; i < inputs.size(); i++) {
            builder.addSlot(RecipeIngredientRole.INPUT, slots1.get(i).getJeiX() - 8, slots1.get(i).getJeiY() - 10).addItemStacks(JeiIngredientHelper.getInputVariants(recipe, i, inputs.get(i)));
