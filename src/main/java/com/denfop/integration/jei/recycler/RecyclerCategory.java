...
public class RecyclerCategory extends GuiIU implements IRecipeCategory<RecyclerHandler> {
    ...
    @Override
    public RecipeType<RecyclerHandler> getRecipeType() {
    ...
    public String getTitles() {
        return Localization.translate(ItemStackHelper.fromData(IUItem.simplemachine, 1, 5).getDescriptionId());
    }
    ...
    @Override
    public void draw(RecyclerHandler recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
