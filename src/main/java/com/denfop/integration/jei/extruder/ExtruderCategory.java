...
public class ExtruderCategory extends ScreenMain implements IRecipeCategory<ExtruderHandler> {
    ...
    public ExtruderCategory(
        ...
        super(new ContainerMenuMultiMachine(Minecraft.getInstance().player,
                ((BlockEntityExtruding) BlockMoreMachine2Entity.extruder.getDummyTe()), 1, true
        ));
    ...
    @Override
    public void draw(ExtruderHandler recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
    ...
    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, ExtruderHandler recipes, IFocusGroup focuses) {
    ...
    @Override
    public RecipeType<ExtruderHandler> getRecipeType() {
