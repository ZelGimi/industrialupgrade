...
public class PlasticCreatorCategory extends GuiIU implements IRecipeCategory<PlasticCreatorHandler> {
    ...
    public String getTitles() {
        return Localization.translate(ItemStackHelper.fromData(IUItem.basemachine1, 1, 11).getDescriptionId());
    }
    ...
    @Override
    public void draw(PlasticCreatorHandler recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
    ...
    @Override
    public RecipeType<PlasticCreatorHandler> getRecipeType() {
