...
public class RadioactiveOreHandlerCategory extends ScreenMain implements IRecipeCategory<RadioactiveOreHandlerHandler> {
    ...
    public String getTitles() {
        return Localization.translate(JEICompat.getBlockStack(BlockBaseMachine3Entity.radioactive_handler_ore).getDescriptionId());
    }
    ...
    @Override
    public void draw(RadioactiveOreHandlerHandler recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics stack, double mouseX, double mouseY) {
        progress++;
        if (this.energy < 100) {
            energy++;
        }
