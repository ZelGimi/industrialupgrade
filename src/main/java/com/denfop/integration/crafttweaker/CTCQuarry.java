...
@ZenClass("mods.industrialupgrade.QuarryCrushedRecipe")
@ModOnly("industrialupgrade")
@ZenRegister
public class CTCQuarry {
    ...
    public static void remove(IItemStack input) {
        CraftTweakerAPI.apply(new CTCQuarry.Remove(input));
    }
    ...
    private static class AddMolecularAction extends BaseAction {
        ...
        public void apply() {
            ...
            if (OreDictionary.getOreIDs((ItemStack) output.getInternal()).length == 0) {
                if (!IUCore.list_crushed_adding.contains((ItemStack) output.getInternal())) {
                    IUCore.list_crushed_adding.add((ItemStack) output.getInternal());
                } else {
                    int i = OreDictionary.getOreIDs((ItemStack) output.getInternal())[0];
                    if (!IUCore.list_crushed_adding.contains(OreDictionary.getOres(OreDictionary.getOreName(i)).get(0))) {
                        IUCore.list_crushed_adding.add(OreDictionary.getOres(OreDictionary.getOreName(i)).get(0));
                    }
                }
            }
    ...
    private static class Remove extends BaseAction {
        ...
        public void apply() {
            if (OreDictionary.getOreIDs((ItemStack) input.getInternal()).length == 0) {
                IUCore.list_crushed_removing.add((ItemStack) input.getInternal());
            } else {
                int i = OreDictionary.getOreIDs((ItemStack) input.getInternal())[0];
                IUCore.list_crushed_removing.add(OreDictionary.getOres(OreDictionary.getOreName(i)).get(0));
            }
        }
