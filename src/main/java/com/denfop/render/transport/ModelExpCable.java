...
public class ModelExpCable extends AbstractModel implements ISpecialParticleModel {

    private final Map<ResourceLocation, TextureAtlasSprite> textures;
    private final LoadingCache<TileEntityExpPipes.CableRenderState, IBakedModel> modelCache;

    public ModelExpCable() {
        textures = generateTextureLocations();
        this.modelCache = CacheBuilder
                .newBuilder()
                .maximumSize(256L)
                .expireAfterAccess(5L, TimeUnit.MINUTES)
                .build(new CacheLoader<TileEntityExpPipes.CableRenderState, IBakedModel>() {
                    public IBakedModel load(@Nonnull TileEntityExpPipes.CableRenderState key) {
                        return ModelExpCable.this.generateModel(key);
                    }
                });
    }
    ...
    private static Map<ResourceLocation, TextureAtlasSprite> generateTextureLocations() {
        ...
        name.append("blocks/wiring/expcable/");
        int reset0 = name.length();
        ExpType[] var3 = ExpType.values;


        for (ExpType type : var3) {
            name.append(type.name());
            name.append("_expcable");
    ...
    private static ResourceLocation getTextureLocation(ExpType type) {
        String loc = "blocks/wiring/expcable/" + type.getName();
    ...
    public List<BakedQuad> getQuads(IBlockState rawState, EnumFacing side, long rand) {
        if (!(rawState instanceof Ic2BlockStateInstance)) {
            ...
            if (!state.hasValue(TileEntityExpPipes.renderStateProperty)) {
                return ModelUtil.getMissingModel().getQuads(state, side, rand);
            } else {
                TileEntityExpPipes.CableRenderState prop = state.getValue(TileEntityExpPipes.renderStateProperty);
    ...
    private IBakedModel generateModel(TileEntityExpPipes.CableRenderState prop) {
