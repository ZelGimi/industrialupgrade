...
import com.denfop.tiles.transport.tiles.TileEntityItemPipes.CableRenderState;
...
public class ModelTransportPipes extends AbstractModel implements ISpecialParticleModel {

    private final Map<ResourceLocation, TextureAtlasSprite> textures;
    private final LoadingCache<TileEntityItemPipes.CableRenderState, IBakedModel> modelCache;
    private Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter;

    public ModelTransportPipes() {
        ...
        this.modelCache = CacheBuilder
                ...
                .build(new CacheLoader<CableRenderState, IBakedModel>() {
                    public IBakedModel load(@Nonnull CableRenderState key) {
                        return ModelTransportPipes.this.generateModel(key);
                    }
                });
    }
    ...
    private static Map<ResourceLocation, TextureAtlasSprite> generateTextureLocations() {
        ...
        name.append("blocks/wiring/item_pipes/");
        int reset0 = name.length();
        ItemType[] var3 = ItemType.values;


        for (ItemType type : var3) {
    ...
    private static ResourceLocation getTextureLocation(ItemType type) {
        String loc = "blocks/wiring/item_pipes/" + type.getName();
    ...
    public List<BakedQuad> getQuads(IBlockState rawState, EnumFacing side, long rand) {
        if (!(rawState instanceof Ic2BlockStateInstance)) {
            ...
            if (!state.hasValue(TileEntityItemPipes.renderStateProperty)) {
                return ModelUtil.getMissingModel().getQuads(state, side, rand);
            } else {
                CableRenderState prop = state.getValue(TileEntityItemPipes.renderStateProperty);
