...
import com.denfop.tiles.transport.tiles.TileEntityHeatColdPipes.CableRenderState;
...
public class ModelCoolHeatPipes extends AbstractModel implements ISpecialParticleModel {
        ...
        this.modelCache = CacheBuilder
                ...
                .build(new CacheLoader<CableRenderState, IBakedModel>() {
                    public IBakedModel load(@Nonnull CableRenderState key) {
                        return ModelCoolHeatPipes.this.generateModel(key);
                    }
                });
    }
    ...
    private static Map<ResourceLocation, TextureAtlasSprite> generateTextureLocations() {
        ...
        name.append("blocks/wiring/heatcold/");
        int reset0 = name.length();
        HeatColdType[] var3 = HeatColdType.values;

        for (HeatColdType type : var3) {
    ...
    private static ResourceLocation getTextureLocation(HeatColdType type) {
        String loc = "blocks/wiring/heatcold/" + type.getName();
    ...
    public List<BakedQuad> getQuads(IBlockState rawState, EnumFacing side, long rand) {
        if (!(rawState instanceof Ic2BlockStateInstance)) {
            ...
            if (!state.hasValue(TileEntityHeatColdPipes.renderStateProperty)) {
                return ModelUtil.getMissingModel().getQuads(state, side, rand);
            } else {
                CableRenderState prop = state.getValue(TileEntityHeatColdPipes.renderStateProperty);
