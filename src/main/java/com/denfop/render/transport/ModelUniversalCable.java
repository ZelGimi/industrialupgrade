...
import com.denfop.tiles.transport.tiles.TileEntityUniversalCable.CableRenderState;
...
public class ModelUniversalCable extends AbstractModel implements ISpecialParticleModel {
        ...
        this.modelCache = CacheBuilder
                ...
                .build(new CacheLoader<CableRenderState, IBakedModel>() {
                    public IBakedModel load(@Nonnull CableRenderState key) {
                        return ModelUniversalCable.this.generateModel(key);
                    }
                });
    }
    ...
    private static Map<ResourceLocation, TextureAtlasSprite> generateTextureLocations() {
        ...
        name.append("blocks/wiring/universal_cable/");
        int reset0 = name.length();
        UniversalType[] var3 = UniversalType.values;


        for (UniversalType type : var3) {
    ...
    private static ResourceLocation getTextureLocation(UniversalType type) {
        String loc = "blocks/wiring/universal_cable/" + type.getName();
    ...
    public List<BakedQuad> getQuads(IBlockState rawState, EnumFacing side, long rand) {
        if (!(rawState instanceof Ic2BlockStateInstance)) {
            ...
            if (!state.hasValue(TileEntityUniversalCable.renderStateProperty)) {
                return ModelUtil.getMissingModel().getQuads(state, side, rand);
            } else {
                CableRenderState prop = state.getValue(TileEntityUniversalCable.renderStateProperty);
