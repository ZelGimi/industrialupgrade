...
public class RotorModel extends EntityModel<Entity> {
    ...
    public void renderToBuffer(
        ...
        for (int i = 0; i < BLADE_COUNT; i++) {
            fullBlades[i].render(poseStack, buffer, packedLight, packedOverlay, red, green, blue, alpha);
        }
    }
    ...
    public void renderDamagedRotor(
        ...
        for (int i = 0; i < BLADE_COUNT; i++) {
            ...
            poseStack.pushPose();
            poseStack.mulPose(Vector3f.XP.rotationDegrees(blade.getTiltX() + wobbleX));
            poseStack.mulPose(Vector3f.YP.rotationDegrees(blade.getTiltY() + wobbleY));
            poseStack.mulPose(Vector3f.ZP.rotationDegrees(blade.getTiltZ() + wobble));
            ...
            case FULL, CHIPPED -> {
                if (crackAEnabled) {
                    crackAFull[index].render(
                            ...
                            crackShade, crackShade, crackShade, 1.0F
                    );
                }
                if (crackBEnabled) {
