...
public interface ISpecialParticleModel extends IBakedModel {
    default boolean needsEnhancing(IBlockState state) {
        return Minecraft.getMinecraft().getRenderItem().getItemModelMesher().getModelManager().getMissingModel().getParticleTexture().getIconName().equals(this.getParticleTexture().getIconName());
    }
