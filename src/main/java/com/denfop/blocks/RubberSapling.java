...
public class RubberSapling extends SaplingBlock {
    public RubberSapling() {
        super(new TreeGrower("rubber", Optional.of(ConfiguredFeaturesGen.RUB_TREE), Optional.empty(), Optional.empty()), Properties.of().mapColor(MapColor.PLANT).noCollission().randomTicks().strength(0.0F).instabreak().sound(SoundType.GRASS));
    }
    ...
    public void advanceTree(ServerLevel pLevel, BlockPos pPos, BlockState pState, RandomSource pRandom) {
        if (pState.getValue(STAGE) == 0) {
            ...
            RUB_TREE_GENERATOR.get().placeInstantly(pLevel, pLevel.getChunkSource().getGenerator(), pState, pRandom, pPos);
        }
