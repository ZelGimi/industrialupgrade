    ...
    public ResourceLocation getTextureLocation(boolean flowing) {
        if (fluidname == null) {
            if (this.name().startsWith("fluidmolten")) {
                ...
                return new ResourceLocation(Constants.MOD_ID, "blocks/fluid/" + this.name().substring(5).toLowerCase() + "_" + type);
            }
        } else {
            if (fluidname.name().startsWith("fluidmolten")) {
                ...
                String type = flowing && this.hasFlowTexture ? "flow" : "still";
                return new ResourceLocation(Constants.MOD_ID, "blocks/fluid/" + fluidname.name().substring(5).toLowerCase() + "_" + type);
            }
        }
    }
