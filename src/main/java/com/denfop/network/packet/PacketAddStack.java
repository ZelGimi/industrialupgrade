...
public class PacketAddStack implements IPacket {
    ...
    public void readPacket(final CustomPacketBuffer customPacketBuffer, final Player entityPlayer) {
        try {
            ...
            if (entityPlayer.getUUID().equals(uuid))
                if (entityPlayer.getLevel().getBlockEntity(pos) instanceof BlockEntityMonitor controller && controller.network != null) {
                    ...
                    if (count > 0) {
                        ...
                        controller.network.addStack(stack.getItem(), stack.getTag(), count);
            ...
            if (entityPlayer.getLevel().getBlockEntity(pos) instanceof BlockEntityPatternMonitor controller && controller.network != null) {
                ItemStack stack = (ItemStack) DecoderHandler.decode(customPacketBuffer);
                int count = controller.network.canAdd(stack);
                if (count > 0) {
                    stack.setCount(count);
            ...
            if (entityPlayer.getLevel().getBlockEntity(pos) instanceof BlockEntityFluidMonitor controller && controller.network != null) {
                FluidStack stack = (FluidStack) DecoderHandler.decode(customPacketBuffer);
                int count = controller.network.canAdd(stack);
                if (count > 0) {
                    stack.setAmount(count);
                    controller.network.addStack(stack.getFluid(), stack.getTag(), count);
