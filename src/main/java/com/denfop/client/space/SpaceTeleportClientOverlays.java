...
public final class SpaceTeleportClientOverlays {
    ...
    private static final IGuiOverlay OVERLAY = new IGuiOverlay() {
        @Override
        public void render(final ForgeGui gui, final PoseStack poseStack, final float partialTick, final int screenWidth, final int screenHeight) {
            ...
            GuiGraphicsCompat graphics = new GuiGraphicsCompat(poseStack);
    ...
    @SubscribeEvent
    public static void register(final RegisterGuiOverlaysEvent event) {
        event.registerAboveAll("space_teleport_overlay", OVERLAY);
    }

    private static void renderHud(final GuiGraphicsCompat graphics, final int w, final int h, final SpaceTeleportClientState state) {
        int panelX = 12;
        int panelY = h - 80;
        int panelW = 252;
        int panelH = 64;
    ...
    private static void renderHudAccentBars(final GuiGraphicsCompat graphics, final int x, final int y, final boolean warning) {
    ...
    private static void renderCircularChargeBar(
            final GuiGraphicsCompat graphics,
            final float cx,
            final float cy,
    ...
    private static void renderFx(final GuiGraphicsCompat graphics, final int w, final int h, final SpaceTeleportClientState state) {
    ...
    private static void renderForwardCharge(final GuiGraphicsCompat graphics, final int w, final int h, final float progress, final long time) {
    ...
    private static void renderReturnCharge(final GuiGraphicsCompat graphics, final int w, final int h, final float progress, final long time) {
        float eased = easeInOutCubic(progress);
        float cx = w * 0.5f;
        float cy = h * 0.5f;
        float base = Math.min(w, h);
    ...
    private static void renderForwardTunnel(final GuiGraphicsCompat graphics, final int w, final int h, final float progress, final long time) {
        float eased = easeInOutCubic(progress);
        float cx = w * 0.5f;
        float cy = h * 0.5f;
        float base = Math.min(w, h);
    ...
    private static void renderReturnTunnel(final GuiGraphicsCompat graphics, final int w, final int h, final float progress, final long time) {
        float eased = easeInOutCubic(progress);
        float cx = w * 0.5f;
        float cy = h * 0.5f;
        float base = Math.min(w, h);
    ...
    private static void renderForwardArrival(final GuiGraphicsCompat graphics, final int w, final int h, final float progress, final long time) {
    ...
    private static void renderReturnArrival(final GuiGraphicsCompat graphics, final int w, final int h, final float progress, final long time) {
        float eased = easeOutCubic(progress);
        float cx = w * 0.5f;
        float cy = h * 0.5f;
        float base = Math.min(w, h);
    ...
    private static void renderChromaticAberration(final GuiGraphicsCompat graphics, final int w, final int h, final float intensity, final boolean returnMode) {
        if (intensity <= 0f) {
            return;
        }
    ...
    private static void drawTechGlyph(final GuiGraphicsCompat graphics, final float x, final float y, final int size, final int color, final int variant) {
        int ix = Mth.floor(x);
        int iy = Mth.floor(y);
    ...
    private static void renderSpaceRupture(
        ...
        for (int i = 0; i < 10; i++) {
            ...
            builder.vertex(matrix, cx, cy, 0).color(r, g, b, alpha).endVertex();
            builder.vertex(matrix, cx + Mth.cos(a1) * radius * 0.35f, cy + Mth.sin(a1) * radius * 0.35f, 0).color(r, g, b, alpha).endVertex();
            builder.vertex(matrix, cx + Mth.cos(a2) * out, cy + Mth.sin(a2) * out, 0).color(r, g, b, 0).endVertex();
        }
    ...
    private static void renderStabilizationGrid(final GuiGraphicsCompat graphics, final int w, final int h, final float alpha) {
        int lines = 14;
    ...
    private static void renderScanlines(final GuiGraphicsCompat graphics, final int w, final int h, final float alpha) {
    ...
    private static void renderNoiseField(final GuiGraphicsCompat graphics, final int w, final int h, final long time, final int count, final float strength) {
        for (int i = 0; i < count; i++) {
    ...
    private static void drawCrossFlare(final GuiGraphicsCompat graphics, final float cx, final float cy, final float length, final float mul) {
        int a1 = (int) (110 * mul);
        int a2 = (int) (72 * mul);
    ...
    private static void drawDiamondPulse(final Matrix4f matrix, final float cx, final float cy, final float radius, final int color) {
        ...
        builder.vertex(matrix, cx, cy - radius, 0).color(r, g, b, 0).endVertex();
        builder.vertex(matrix, cx + radius, cy, 0).color(r, g, b, 0).endVertex();
        builder.vertex(matrix, cx, cy + radius, 0).color(r, g, b, 0).endVertex();
        builder.vertex(matrix, cx - radius, cy, 0).color(r, g, b, 0).endVertex();
        builder.vertex(matrix, cx, cy - radius, 0).color(r, g, b, 0).endVertex();
        tessellator.end();
    ...
    private static void drawHelixBand(
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            builder.vertex(matrix, x, y, 0).color(r, g, b, alpha).endVertex();
        }
    ...
    private static void drawShard(
        ...
        builder.begin(VertexFormat.Mode.TRIANGLES, DefaultVertexFormat.POSITION_COLOR);
        builder.vertex(matrix, ex, ey, 0).color(r, g, b, 0).endVertex();
        builder.vertex(matrix, sx + px * width, sy + py * width, 0).color(r, g, b, a).endVertex();
        builder.vertex(matrix, sx - px * width, sy - py * width, 0).color(r, g, b, a).endVertex();
        tessellator.end();
    ...
    private static void drawSoftDisc(final Matrix4f matrix, final float cx, final float cy, final float radius, final int innerColor, final int outerColor) {
        ...
        builder.begin(VertexFormat.Mode.TRIANGLE_FAN, DefaultVertexFormat.POSITION_COLOR);
        builder.vertex(matrix, cx, cy, 0).color(ir, ig, ib, ia).endVertex();
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            float x = cx + Mth.cos(angle) * radius;
            float y = cy + Mth.sin(angle) * radius;
            builder.vertex(matrix, x, y, 0).color(or, og, ob, oa).endVertex();
        }
    ...
    private static void drawRingArc(
        ...
        for (int i = 0; i <= segments; i++) {
            ...
            builder.vertex(matrix, cx + cos * outerRadius, cy + sin * outerRadius, 0).color(r, g, b, a).endVertex();
            builder.vertex(matrix, cx + cos * innerRadius, cy + sin * innerRadius, 0).color(r, g, b, a).endVertex();
        }
    ...
    private static void drawBeam(
        ...
        builder.vertex(matrix, ix + px * widthInner, iy + py * widthInner, 0).color(ir, ig, ib, ia).endVertex();
        builder.vertex(matrix, ix - px * widthInner, iy - py * widthInner, 0).color(ir, ig, ib, ia).endVertex();
        builder.vertex(matrix, ox - px * widthOuter, oy - py * widthOuter, 0).color(or, og, ob, oa).endVertex();
        builder.vertex(matrix, ox + px * widthOuter, oy + py * widthOuter, 0).color(or, og, ob, oa).endVertex();
        tessellator.end();
    ...
    private static void drawFilledQuad(final Matrix4f matrix, final int minX, final int minY, final int maxX, final int maxY, final int color) {
        int left = Math.min(minX, maxX);
        int right = Math.max(minX, maxX);
        int top = Math.min(minY, maxY);
        int bottom = Math.max(minY, maxY);
        ...
        builder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        builder.vertex(matrix, left, bottom, 0).color(r, g, b, a).endVertex();
        builder.vertex(matrix, right, bottom, 0).color(r, g, b, a).endVertex();
        builder.vertex(matrix, right, top, 0).color(r, g, b, a).endVertex();
        builder.vertex(matrix, left, top, 0).color(r, g, b, a).endVertex();
        tessellator.end();

        RenderSystem.disableBlend();
    }

    private static void drawVerticalGradientQuad(
            final Matrix4f matrix,
            final int minX,
            final int minY,
            final int maxX,
        ...
        int ta = (topColor >> 24) & 255;
        int tr = (topColor >> 16) & 255;
        int tg = (topColor >> 8) & 255;
        int tb = topColor & 255;

        int ba = (bottomColor >> 24) & 255;
        ...
        builder.begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        builder.vertex(matrix, left, bottom, 0).color(br, bg, bb, ba).endVertex();
        builder.vertex(matrix, right, bottom, 0).color(br, bg, bb, ba).endVertex();
        builder.vertex(matrix, right, top, 0).color(tr, tg, tb, ta).endVertex();
        builder.vertex(matrix, left, top, 0).color(tr, tg, tb, ta).endVertex();
        tessellator.end();
    ...
    private static final class GuiGraphicsCompat {
        private final PoseStack poseStack;

        private GuiGraphicsCompat(final PoseStack poseStack) {
        ...
        private void fill(final int minX, final int minY, final int maxX, final int maxY, final int color) {
            drawFilledQuad(this.poseStack.last().pose(), minX, minY, maxX, maxY, color);
        }

        private void fillGradient(final int minX, final int minY, final int maxX, final int maxY, final int topColor, final int bottomColor) {
            drawVerticalGradientQuad(this.poseStack.last().pose(), minX, minY, maxX, maxY, topColor, bottomColor);
        }

        private void drawString(final Font font, final String text, final int x, final int y, final int color, final boolean shadow) {
            if (shadow) {
