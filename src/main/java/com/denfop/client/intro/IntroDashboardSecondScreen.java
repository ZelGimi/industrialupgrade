...
public class IntroDashboardSecondScreen extends Screen {
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick) {
        IntroGuiGraphics guiGraphics = new IntroGuiGraphics(poseStack);
    ...
    private void renderPageHeader(IntroGuiGraphics guiGraphics, Layout l) {
    ...
    private void drawNavButton(IntroGuiGraphics guiGraphics, Rect rect, String label, boolean active) {
    ...
    private void renderLanguagesPanel(IntroGuiGraphics guiGraphics, Rect panel) {
        ...
        String currentCode = Minecraft.getInstance().getLanguageManager().getSelected().getCode();
    ...
    private void drawLanguageProgressBar(IntroGuiGraphics guiGraphics, int x, int y, int w, int h, float progress, int fillColor) {
    ...
    private void drawLanguageSmallBadge(IntroGuiGraphics guiGraphics, int x, int y, int w, String label, int fillColor) {
        int h = 10;
    ...
    private List<Component> renderFaqPanel(IntroGuiGraphics guiGraphics, ScrollArea panel, int mouseX, int mouseY) {
    ...
    private List<Component> renderFormerWorkersPanel(IntroGuiGraphics guiGraphics, ScrollArea panel, int mouseX, int mouseY) {
        IntroDrawUtil.drawPanel(guiGraphics, panel.panelX(), panel.panelY(), panel.panelW(), panel.panelH(), PANEL_FILL, PANEL_BORDER);
    ...
    private List<Component> renderBillboardPanel(IntroGuiGraphics guiGraphics, Rect panel, int mouseX, int mouseY) {
        IntroDrawUtil.drawPanel(guiGraphics, panel.x(), panel.y(), panel.w(), panel.h(), PANEL_FILL, PANEL_BORDER);
    ...
    private void renderBillboardSlide(IntroGuiGraphics guiGraphics, IntroBillboardSlide slide, Rect viewport, float alpha) {
    ...
    private void renderChangelogPanel(IntroGuiGraphics guiGraphics, ScrollArea panel) {
        IntroDrawUtil.drawPanel(guiGraphics, panel.panelX(), panel.panelY(), panel.panelW(), panel.panelH(), PANEL_FILL, PANEL_BORDER);
    ...
    private void renderFullChangelogModal(IntroGuiGraphics guiGraphics, Layout l) {
    ...
    private void drawWrapBadge(IntroGuiGraphics guiGraphics, int centerX, int y, int maxWidth, String label, int fillColor, float scale) {
    ...
    private void drawCenteredText(IntroGuiGraphics guiGraphics, String label, int centerX, int y, int color) {
        int w = font.width(label);
        guiGraphics.drawString(font, label, centerX - w / 2, y, color, false);
    }

    private void drawCenteredScaledText(IntroGuiGraphics guiGraphics, String label, int centerX, int y, float scale, int color) {
    ...
    private void drawCenteredScaledLine(IntroGuiGraphics guiGraphics, FormattedCharSequence label, int centerX, int y, float scale, int color) {
        int scaledW = Math.round(font.width(label) * scale);
        int x = centerX - scaledW / 2;
        guiGraphics.pose().pushPose();
        guiGraphics.pose().translate(x, y, 0);
    ...
    private void renderLanguagesScrollbar(IntroGuiGraphics guiGraphics, Rect panel) {
    ...
    private void drawMarqueeText(IntroGuiGraphics guiGraphics, String label, int x, int y, int width, int color, long now) {
    ...
    private void renderScrollBar(IntroGuiGraphics guiGraphics, ScrollArea panel, int scroll, int contentHeight) {
        int maxScroll = Math.max(0, contentHeight - panel.contentH());
        if (maxScroll <= 0) return;
