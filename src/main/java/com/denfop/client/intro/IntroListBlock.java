...
public class IntroListBlock extends IntroMarkdownBlock {
    ...
    @Override
    public void render(IntroGuiGraphics guiGraphics, Font font, IntroImageManager imageManager, int x, int y, int width, int mouseX, int mouseY, float partialTick) {
        int yy = y;
        int textX = x + 12;
        int innerWidth = Math.max(20, width - 12);
